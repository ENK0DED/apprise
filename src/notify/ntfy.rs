...
mod tests {
    ...
    async fn test_bearer_token_auth() {
        ...
        let addr = server.address();
        let url_str = format!(
            "ntfy://tk_mytoken123@localhost:{}/mytopic",
            addr.port()
        );
        let parsed = ParsedUrl::parse(&url_str).unwrap();
        let ntfy = Ntfy::from_url(&parsed).unwrap();
    ...
    async fn test_basic_auth() {
        ...
        let expected = format!(
            "Basic {}",
            base64::engine::general_purpose::STANDARD.encode("myuser:mypass")
        );
        Mock::given(method("POST"))
            .and(path("/mytopic"))
        ...
        let addr = server.address();
        let url_str = format!(
            "ntfy://myuser:mypass@localhost:{}/mytopic",
            addr.port()
        );
        let parsed = ParsedUrl::parse(&url_str).unwrap();
        let ntfy = Ntfy::from_url(&parsed).unwrap();
    ...
    async fn test_token_auth_via_query_param() {
        ...
        let addr = server.address();
        let url_str = format!(
            "ntfy://localhost:{}/mytopic?token=abc1234",
            addr.port()
        );
        let parsed = ParsedUrl::parse(&url_str).unwrap();
        let ntfy = Ntfy::from_url(&parsed).unwrap();
    ...
    async fn test_auth_token_mode_uses_password_as_token() {
        ...
        let addr = server.address();
        let url_str = format!(
            "ntfy://user:secrettoken@localhost:{}/mytopic?auth=token",
            addr.port()
        );
        let parsed = ParsedUrl::parse(&url_str).unwrap();
        let ntfy = Ntfy::from_url(&parsed).unwrap();
    ...
    async fn test_high_priority_header() {
        ...
        let addr = server.address();
        let url_str = format!(
            "ntfy://localhost:{}/mytopic?priority=high",
            addr.port()
        );
        let parsed = ParsedUrl::parse(&url_str).unwrap();
        let ntfy = Ntfy::from_url(&parsed).unwrap();
    ...
    async fn test_max_priority_header() {
        ...
        let addr = server.address();
        let url_str = format!(
            "ntfy://localhost:{}/mytopic?priority=max",
            addr.port()
        );
        let parsed = ParsedUrl::parse(&url_str).unwrap();
        let ntfy = Ntfy::from_url(&parsed).unwrap();
    ...
    async fn test_numeric_priority() {
        ...
        let addr = server.address();
        let url_str = format!(
            "ntfy://localhost:{}/mytopic?priority=1",
            addr.port()
        );
        let parsed = ParsedUrl::parse(&url_str).unwrap();
        let ntfy = Ntfy::from_url(&parsed).unwrap();

...
pub struct SparkPost { api_key: String, from: String, targets: Vec<String>, pub(crate) host: String, pub(crate) scheme: String, verify_certificate: bool, tags: Vec<String> }
impl SparkPost {
...
mod tests {
    ...
    fn test_from_url_fields() {
        let parsed = crate::utils::parse::ParsedUrl::parse(
            "sparkpost://user@myapikey/recipient@example.com"
        ).unwrap();
    ...
    fn test_from_url_non_email_target() {
        let parsed = crate::utils::parse::ParsedUrl::parse(
            &format!("sparkpost://user@apikey/{}/invalid", "a".repeat(32))
        ).unwrap();
        let sp = SparkPost::from_url(&parsed).unwrap();
    ...
    fn test_region_validation() {
        // US region valid
        let parsed = crate::utils::parse::ParsedUrl::parse(
            "sparkpost://user@apikey/target?region=us"
        ).unwrap();
        assert!(SparkPost::from_url(&parsed).is_some());

        // EU region valid
        let parsed = crate::utils::parse::ParsedUrl::parse(
            "sparkpost://user@apikey/target?region=EU"
        ).unwrap();
        assert!(SparkPost::from_url(&parsed).is_some());

        // Invalid region
        let parsed = crate::utils::parse::ParsedUrl::parse(
            "sparkpost://user@apikey/target?region=invalid"
        ).unwrap();
    ...
    fn sparkpost_for_mock(server: &MockServer) -> SparkPost {
        ...
        let parsed = crate::utils::parse::ParsedUrl::parse(
            "sparkpost://user@apikey/recipient@example.com"
        ).unwrap();
    ...
    async fn test_send_multiple_recipients() {
        ...
        let addr = server.address();
        let parsed = crate::utils::parse::ParsedUrl::parse(
            "sparkpost://user@apikey/user1@example.com/user2@example.com"
        ).unwrap();
        let mut sp = SparkPost::from_url(&parsed).unwrap();
        sp.host = format!("{}:{}", addr.ip(), addr.port());
        sp.scheme = "http".to_string();

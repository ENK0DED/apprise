...
pub struct Resend { apikey: String, from_email: String, to: Vec<String>, cc: Vec<String>, bcc: Vec<String>, reply_to: Option<String>, verify_certificate: bool, tags: Vec<String> }
impl Resend {
    ...
    pub fn static_details() -> ServiceDetails { ServiceDetails { service_name: "Resend", service_url: Some("https://resend.com"), setup_url: None, protocols: vec!["resend"], description: "Send email via Resend.", attachment_support: true } }
}
#[async_trait]
impl Notify for Resend {
    ...
    async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
        ...
        for target in &self.to {
            ...
            if !self.cc.is_empty() { payload["cc"] = json!(self.cc); }
            if !self.bcc.is_empty() { payload["bcc"] = json!(self.bcc); }
            if let Some(ref r) = self.reply_to { payload["reply_to"] = json!(r); }

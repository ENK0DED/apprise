...
pub use error::{NotifyError, ConfigError, AttachError};
pub use notify::{Notify, NotifyContext, Attachment, ServiceDetails, OverflowMode};
pub use notify::registry;
pub use types::{NotifyType, NotifyFormat, StorageMode};
...
impl Apprise {
    ...
    pub async fn notify(&self, ctx: &NotifyContext) -> SendResult {
        ...
        let work: Vec<(usize, Vec<NotifyContext>)> = self.filtered_indices()
            .into_iter()
            .map(|i| (i, Self::prepare_contexts(self.services[i].as_ref(), ctx)))
            .collect();
        let total = work.len();
    ...
    pub async fn notify_sequential(&self, ctx: &NotifyContext) -> SendResult {
        ...
        for i in &indices {
            ...
            for svc_ctx in &contexts {
                ...
                match svc.send(svc_ctx).await {
                    Ok(true) => {}
                    Ok(false) | Err(_) => { svc_ok = false; }
                }
            }
            if svc_ok { succeeded += 1; } else { failed += 1; }
        }
    ...
    fn filtered_indices(&self) -> Vec<usize> {
        ...
        self.services.iter().enumerate().filter_map(|(i, svc)| {
            let svc_tags: Vec<String> = svc.tags().iter().map(|t| t.to_lowercase()).collect();
            if svc_tags.iter().any(|t| t == "always") { return Some(i); }

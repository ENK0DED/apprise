...
/// Return type for config loading functions.
type LoadConfigResult<'a> = std::pin::Pin<Box<dyn std::future::Future<Output = Result<(Vec<Box<dyn Notify>>, Option<AppriseAsset>), ConfigError>> + Send + 'a>>;
...
fn source_protocol(source: &str) -> &'static str {
  if source.starts_with("http://") || source.starts_with("https://") { "http" } else { "file" }
}
...
pub fn detect_format(source: &str) -> ConfigFormat {
  ...
  if path_part.ends_with(".yaml") || path_part.ends_with(".yml") || path_part.contains(".yaml") || path_part.contains(".yml") {
    ConfigFormat::Yaml
  } else {
    ConfigFormat::Text
  }
...
pub fn load_config(source: &str, recursion_depth: u32) -> LoadConfigResult<'_> {
  ...
  Box::pin(async move {
    ...
    if let Some(parent) = parent_source {
      if !cross_include_allowed(parent, source, cross_include_mode) {
        tracing::warn!("Cross-include blocked: {} -> {} (mode={:?})", parent, source, cross_include_mode,);

...
fn parse_tag_value(val: &serde_yaml::Value) -> Option<String> {
  if let Some(s) = val.as_str() {
    ...
    let tags: Vec<String> = seq.iter().filter_map(|t| t.as_str().map(|s| s.trim().to_string())).filter(|s| !s.is_empty()).collect();
...
fn parse_global_tags(doc: &serde_yaml::Value) -> Option<String> {
  doc.get("tag").or_else(|| doc.get("tags")).and_then(parse_tag_value)
}
    ...
    for (k, v) in g {
      if let Some(name) = k.as_str() {
        let members = if let Some(s) = v.as_str() {
          ...
          seq.iter().filter_map(|t| t.as_str().map(|s| s.trim().to_string())).filter(|t| !t.is_empty()).collect()
        } else {
          Vec::new()
        };
        ...
        if let Some(s) = item.as_str() {
          ...
          let instances: Vec<Option<&serde_yaml::Mapping>> = if let Some(val) = value {
            if let Some(seq) = val.as_sequence() {
              ...
              seq.iter().map(|entry| entry.as_mapping()).collect()
...
"#;
  ...
  fn test_parse_tag_value_sequence() {
    let v = serde_yaml::Value::Sequence(vec![serde_yaml::Value::String("x".to_string()), serde_yaml::Value::String("y".to_string())]);
  ...
  fn test_append_overrides_to_url() {
    ...
    m.insert(serde_yaml::Value::String("to".to_string()), serde_yaml::Value::String("user@example.com".to_string()));
    m.insert(serde_yaml::Value::String("tag".to_string()), serde_yaml::Value::String("ignored".to_string()));

...
pub async fn parse_text(content: &str, recursion_depth: u32) -> Result<Vec<Box<dyn Notify>>, ConfigError> {
  ...
  for line in content.lines() {
    ...
    let trimmed = trimmed.find(" #").or_else(|| trimmed.find(" ;")).map(|i| trimmed[..i].trim()).unwrap_or(trimmed);
    ...
    let (tags, url_str) = if let Some(eq_pos) = trimmed.find('=') {
      ...
      if !left.contains("://") && right.contains("://") { (Some(left.to_string()), right.trim().to_string()) } else { (None, trimmed.to_string()) }
    } else {

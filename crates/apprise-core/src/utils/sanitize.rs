...
const BLOB_KEYWORDS: &[&str] =
  &["base64", "attachment", "attachments", "base64_attachments", "contentbytes", "blob", "file", "data", "image", "media", "document"];
...
impl Default for SanitizeOptions {
  fn default() -> Self {
    Self { max_depth: 10, max_items: 100, max_str_len: 512, preview: 32, hash_sample_size: 8192, aggressive_blob_keys: true }
  }
}
...
pub fn sanitize_payload_with_options(value: &serde_json::Value, opts: &SanitizeOptions) -> serde_json::Value {
  let mut items_seen: usize = 0;
  walk(value, 0, false, opts, &mut items_seen)
}

fn _hash_preview(s: &str, sample_size: usize) -> String {
...
fn summarize_str(s: &str, opts: &SanitizeOptions, blob_mode: bool) -> serde_json::Value {
  ...
  if blob_mode && opts.aggressive_blob_keys {
    ...
    let tail: String = if len >= opts.preview { s.chars().skip(len.saturating_sub(opts.preview)).collect() } else { s.to_string() };
    return serde_json::Value::String(format!("<string len={} blob head={:?} tail={:?}>", len, head, tail));
  }
  ...
  let head: String = s.chars().take(opts.preview).collect();
  let tail: String = if len >= opts.preview { s.chars().skip(len.saturating_sub(opts.preview)).collect() } else { s.to_string() };
  serde_json::Value::String(format!("<string len={} head={:?} tail={:?}>", len, head, tail))
}
...
fn walk(value: &serde_json::Value, depth: usize, blob_mode: bool, opts: &SanitizeOptions, items_seen: &mut usize) -> serde_json::Value {

...
impl Mastodon {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://mastodon.social"),
      setup_url: None,
      protocols: vec!["mastodon", "toot", "mastodons", "toots"],
      description: "Post a toot on Mastodon.",
...
impl Notify for Mastodon {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    for att in &ctx.attachments {
      ...
      let upload_resp =
        client.post(&upload_url).header("User-Agent", APP_ID).header("Authorization", format!("Bearer {}", self.token)).multipart(form).send().await?;
    ...
    if let Some(ref spoiler) = self.spoiler_text {
    ...
    let resp = client.post(&status_url).header("User-Agent", APP_ID).header("Authorization", format!("Bearer {}", self.token)).json(&payload).send().await?;
...
mod tests {
  ...
  async fn test_send_status_failure() {
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/api/v1/statuses")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_send_with_attachment() {
    ...
    c.attachments.push(crate::notify::Attachment { name: "test.gif".to_string(), data: b"GIF89a".to_vec(), mime_type: "image/gif".to_string() });
  ...
  async fn test_unsupported_attachment_skipped() {
    ...
    let mut c = ctx("title", "body");
    c.attachments.push(crate::notify::Attachment { name: "archive.zip".to_string(), data: b"PK".to_vec(), mime_type: "application/zip".to_string() });

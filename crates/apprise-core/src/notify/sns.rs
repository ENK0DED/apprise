...
impl Sns {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let (access_key, secret_key, region, targets) = if url.password.is_some() {
      ...
      if url.path_parts.len() < 2 {
        return None;
      }
      let sk = url.path_parts.first()?.clone();
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://aws.amazon.com/sns/"),
      setup_url: None,
      protocols: vec!["sns"],
      description: "Send notifications via AWS SNS.",
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["sns://", "sns://:@/", "sns://T1JJ3T3L2", "sns://T1JJ3TD4JD/TIiajkdnlazk7FQ/"];
    for url in &urls {
  ...
  fn test_from_url_path_form_fields() {
    // sns://ACCESS_KEY/SECRET_KEY/REGION/TARGET
    let parsed = crate::utils::parse::ParsedUrl::parse("sns://AHIAJGNT76XIMXDBIJYA/bu1dHSdO22pfaaVy/us-east-2/12223334444").unwrap();
  ...
  fn test_from_url_query_form_fields() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sns://?access=MYKEY&secret=MYSECRET&region=us-west-2&to=12223334444").unwrap();
    let sns = Sns::from_url(&parsed).unwrap();
  ...
  fn test_from_url_multiple_targets() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sns://T1JJ3TD4JD/TIiajkdnlazk7FQ/us-west-2/12223334444/12223334445").unwrap();
    let sns = Sns::from_url(&parsed).unwrap();
  ...
  fn test_from_url_with_to_param() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sns://T1JJ3T3L2/A1BRTD4JD/TIiajkdnlazkcOXrIdevi7FQ/us-east-1?to=12223334444").unwrap();
    let sns = Sns::from_url(&parsed).unwrap();
    assert!(sns.targets.contains(&"12223334444".to_string()));
  }
  ...
  fn test_from_url_no_recipients() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sns://AHIAJGNT76XIMXDBIJYA/bu1dHSdO22pfaaVy/us-east-2/").unwrap();
    let sns = Sns::from_url(&parsed).unwrap();

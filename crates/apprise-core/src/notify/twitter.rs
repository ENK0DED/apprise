...
impl Twitter {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let (consumer_key, consumer_secret, access_token, access_token_secret, targets) = if url.password.is_some() {
      ...
      let at = url.path_parts.first()?.clone();
      ...
      let cs = url.path_parts.first()?.clone();
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://twitter.com"),
      setup_url: None,
      protocols: vec!["twitter", "x", "tweet"],
      description: "Send tweets or DMs via Twitter/X API.",
...
impl Notify for Twitter {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    for att in &ctx.attachments {
      ...
      let upload_resp = client.post(upload_url).header("User-Agent", APP_ID).header("Authorization", auth).multipart(form).send().await?;
...
mod tests {
  ...
  fn test_from_url_with_targets() {
    ...
    let parsed = ParsedUrl::parse("twitter://usera@consumer_key/consumer_secret/atoken14/access_secret/user/?to=userb").unwrap();
    let tw = Twitter::from_url(&parsed).unwrap();

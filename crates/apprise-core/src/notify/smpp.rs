...
impl Smpp {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: None,
      setup_url: None,
      protocols: vec!["smpp", "smpps"],
      description: "Send SMS via SMPP protocol.",
...
impl Notify for Smpp {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let mut stream = TcpStream::connect(&addr).await.map_err(|e| NotifyError::Other(format!("SMPP connect to {} failed: {}", addr, e)))?;
    ...
    stream.write_all(&make_pdu(0x00000002, 1, &bind_body)).await.map_err(|e| NotifyError::Other(e.to_string()))?;
    ...
    for target in &self.targets {
      ...
      stream.write_all(&make_pdu(0x00000004, seq, &body)).await.map_err(|e| NotifyError::Other(e.to_string()))?;

...
impl Json {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let payload_extras: Vec<(String, String)> = url.qsd.iter().filter(|(k, _)| k.starts_with(':')).map(|(k, v)| (k[1..].to_string(), v.clone())).collect();
    Some(Self {
      host,
      port: url.port,
      path,
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: None,
      setup_url: None,
      protocols: vec!["json", "jsons"],
      description: "Send a JSON notification to any HTTP endpoint.",
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["json://:@/", "json://", "jsons://", "json://user@localhost?method=invalid"];
    for url in &urls {
  ...
  async fn test_custom_key_mappings() {
    ...
    Mock::given(method("GET")).and(path("/command")).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;

    // :message=msg remaps "message" to "msg"
    // :type= removes the "type" field
    // :test=value adds a new field "test"="value"
    let url = format!("json://localhost:{}/command?:message=msg&:test=value&method=GET&:type=", port);
    let j = json_for_mock(&server, &url);
    let result = j.send(&ctx("title", "body")).await;
    assert!(result.is_ok());
    assert!(result.unwrap());
  ...
  async fn test_type_field_present_when_not_removed() {
    ...
    let url = format!("json://localhost:{}/command?:message=msg&method=GET", port);
    let j = json_for_mock(&server, &url);
    let result = j.send(&ctx("title", "body")).await;
    assert!(result.is_ok());
    assert!(result.unwrap());
  ...
  async fn test_custom_port_and_path() {
    ...
    Mock::given(method("POST")).and(path("/some/path")).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;
  ...
  async fn test_single_attachment_base64() {
    ...
    c.attachments.push(crate::notify::Attachment { name: "apprise-test.gif".to_string(), data: file_data.to_vec(), mime_type: "image/gif".to_string() });
  ...
  async fn test_http_500_returns_error() {
    ...
    Mock::given(method("POST")).and(path("/")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_basic_auth_header() {
    ...
    let expected_auth = format!("Basic {}", base64::engine::general_purpose::STANDARD.encode("user:pass"));
    Mock::given(method("POST"))
      .and(path("/"))
  ...
  async fn test_notify_type_values() {
    for (ntype, expected) in &[(NotifyType::Info, "info"), (NotifyType::Success, "success"), (NotifyType::Warning, "warning"), (NotifyType::Failure, "failure")]
    {
      let server = MockServer::start().await;
      let port = server.address().port();

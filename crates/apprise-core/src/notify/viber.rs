...
impl Viber {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    let token = url.host.clone().filter(|h| !h.is_empty()).or_else(|| url.get("token").map(|s| s.to_string())).unwrap_or_default();
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Viber",
      service_url: Some("https://www.viber.com"),
      setup_url: None,
      protocols: vec!["viber"],
      description: "Send messages via Viber Bot API.",

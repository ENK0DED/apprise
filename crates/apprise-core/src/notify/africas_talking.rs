...
impl AfricasTalking {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let user = url.get("user").map(|s| s.to_string()).or_else(|| url.user.clone()).unwrap_or_else(|| "sandbox".to_string());
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://africastalking.com"),
      setup_url: None,
      protocols: vec!["atalk"],
      description: "Send SMS via Africa's Talking.",
...
impl Notify for AfricasTalking {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let resp = client
      ...
      .header("apiKey", self.apikey.as_str())
...
mod tests {
  ...
  fn test_from_url_struct_fields() {
    let parsed = crate::utils::parse::ParsedUrl::parse("atalk://myuser@myapikey/+15551231234/15555555555").unwrap();
  ...
  fn test_from_url_query_params() {
    let parsed = crate::utils::parse::ParsedUrl::parse("atalk://_?user=testuser&apikey=testapikey&to=11111111111,22222222222").unwrap();
    let obj = AfricasTalking::from_url(&parsed).unwrap();
  ...
  fn test_from_url_key_alias() {
    let parsed = crate::utils::parse::ParsedUrl::parse("atalk://_?user=testuser&key=mykeyvalue&to=11111111111").unwrap();
    let obj = AfricasTalking::from_url(&parsed).unwrap();
  ...
  fn test_from_url_no_targets_returns_none() {
    let parsed = crate::utils::parse::ParsedUrl::parse("atalk://user@apikey").unwrap();
    assert!(AfricasTalking::from_url(&parsed).is_none());
  }

  #[test]
  fn test_mode_sandbox() {
    let parsed = crate::utils::parse::ParsedUrl::parse("atalk://user@apikey/+44444444444?mode=s").unwrap();
    assert!(AfricasTalking::from_url(&parsed).is_some());
  }

  #[test]
  fn test_mode_premium() {
    let parsed = crate::utils::parse::ParsedUrl::parse("atalk://user@apikey/+44444444444?mode=PREM").unwrap();
    assert!(AfricasTalking::from_url(&parsed).is_some());
  }

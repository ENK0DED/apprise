...
impl Fluxer {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: None,
      setup_url: None,
      protocols: vec!["fluxer", "fluxers"],
      description: "Send via Fluxer webhooks.",
...
impl Notify for Fluxer {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let resp = if !ctx.attachments.is_empty() {
      ...
      let mut form = reqwest::multipart::Form::new().text("payload_json", payload_str);
...
mod tests {
  ...
  fn test_from_url_mode_validation() {
    ...
    let parsed = crate::utils::parse::ParsedUrl::parse(&format!("fluxer://{}/{}?mode=private", wid, wtk)).unwrap();
    assert!(Fluxer::from_url(&parsed).is_some());

    // invalid mode returns None
    let parsed = crate::utils::parse::ParsedUrl::parse(&format!("fluxer://{}/{}?mode=invalid", wid, wtk)).unwrap();
    assert!(Fluxer::from_url(&parsed).is_none());
  }
  ...
  fn test_from_url_flags_validation() {
    ...
    // Negative flags
    let parsed = crate::utils::parse::ParsedUrl::parse(&format!("fluxer://{}/{}?flags=-1", wid, wtk)).unwrap();
    assert!(Fluxer::from_url(&parsed).is_none());
  }

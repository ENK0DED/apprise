...
impl Synology {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let token = url.path_parts.first().cloned().or_else(|| url.get("token").map(|s| s.to_string()))?;
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://www.synology.com"),
      setup_url: None,
      protocols: vec!["synology", "synologys"],
      description: "Send via Synology Chat.",
...
impl Notify for Synology {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let params = [("api", "SYNO.Chat.External"), ("method", "incoming"), ("version", "2"), ("token", self.token.as_str())];
    let url = format!("{}://{}:{}/webapi/entry.cgi", schema, self.host, self.port);

    let mut req = client.post(&url).header("User-Agent", APP_ID).query(&params).body(format!("payload={}", urlencoding::encode(&payload_str)));
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["synology://:@/", "synology://", "synologys://", "synology://user@localhost"];
    for url in &urls {
  ...
  async fn test_send_title_and_body_concatenation() {
    ...
    Mock::given(method("POST")).and(path("/webapi/entry.cgi")).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;
  ...
  async fn test_send_server_error_returns_false() {
    ...
    Mock::given(method("POST")).and(path("/webapi/entry.cgi")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_send_bizarre_status_code() {
    ...
    Mock::given(method("POST")).and(path("/webapi/entry.cgi")).respond_with(ResponseTemplate::new(418)).expect(1).mount(&server).await;

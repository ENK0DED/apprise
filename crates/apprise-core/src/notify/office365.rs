...
impl Office365 {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    if let Some(ref user) = url.user {
      ...
      if let Some(password) = &url.password {
        ...
        let client_secret =
          url.path_parts.get(1..).unwrap_or(&[]).iter().take_while(|s| !s.starts_with('@') && !s.contains('@')).cloned().collect::<Vec<_>>().join("/");
      ...
      if tenant.contains(',') || tenant.chars().all(|c| c == '.') {
        return None;
      }
      let client_id = url.path_parts.get(1)?.clone();
      // Validate client_id: reject trailing dots
      if client_id.ends_with('.') {
        return None;
      }
      let client_secret =
        url.path_parts.get(2..).unwrap_or(&[]).iter().take_while(|s| !s.starts_with('@') && !s.contains('@')).cloned().collect::<Vec<_>>().join("/");
      if client_secret.is_empty() {
        return None;
      }
      let secret_parts = client_secret.split('/').count();
    ...
    let secret_parts: Vec<&String> = remaining.iter().take_while(|s| !s.starts_with('@') && !s.contains('@')).collect();
    ...
    let from = after_secret.iter().find(|s| s.contains('@')).cloned().unwrap_or_else(|| after_secret.first().cloned().unwrap_or_default());
    let targets = if after_secret.is_empty() { Vec::new() } else { after_secret };
    if targets.is_empty() {
      return None;
    }
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Office365 Email",
      service_url: Some("https://office.com"),
      setup_url: None,
      protocols: vec!["o365", "azure"],
      description: "Send email via Office 365 / Microsoft Graph.",
...
mod tests {
  ...
  fn test_from_url_email_format() {
    let parsed = ParsedUrl::parse("o365://user@example.edu/tenant/ab-cd-ef-gh/abcd/123/3343/@jack/test/email1@test.ca").expect("parse");
  ...
  fn test_from_url_query_params() {
    let parsed =
      ParsedUrl::parse("o365://_/?oauth_id=ab-cd-ef-gh&oauth_secret=mysecret&tenant=mytenant&to=email1@test.ca&from=user@example.ca").expect("parse");
    let o = Office365::from_url(&parsed).expect("from_url");
    assert_eq!(o.client_id, "ab-cd-ef-gh");
  ...
  fn test_azure_schema_alias() {
    assert!(from_url("azure://user@example.edu/tenant/ab-cd-ef-gh/abcd/123/3343/@jack/test/email1@test.ca").is_some());
  }

  #[test]
  fn test_cc_bcc_from_query() {
    let parsed = ParsedUrl::parse("o365://user@example.com/tenant/ab-cd-ef-gh/abcd/email1@test.ca?cc=cc@test.com&bcc=bcc@test.com").expect("parse");
    let o = Office365::from_url(&parsed).expect("from_url");
  ...
  fn test_token_url_format() {
    ...
    let parsed = ParsedUrl::parse("o365://user@example.com/ff-gg-hh-ii-jj/ab-cd-ef-gh/abcd/email1@test.ca").expect("parse");
    let o = Office365::from_url(&parsed).expect("from_url");
    let expected_token_url = format!("https://login.microsoftonline.com/{}/oauth2/v2.0/token", o.tenant);
  ...
  fn test_send_mail_url_format() {
    ...
    let parsed = ParsedUrl::parse("o365://user@example.net/ff-gg-hh-ii-jj/ab-cd-ef-gh/abcd/target@example.com").expect("parse");
    let o = Office365::from_url(&parsed).expect("from_url");
    let expected_send_url = format!("https://graph.microsoft.com/v1.0/users/{}/sendMail", o.from);

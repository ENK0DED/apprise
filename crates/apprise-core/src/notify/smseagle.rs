...
impl SmsEagle {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "SMSEagle",
      service_url: Some("https://smseagle.eu"),
      setup_url: None,
      protocols: vec!["smseagle", "smseagles"],
      description: "Send SMS via SMSEagle hardware gateway.",
...
mod tests {
  ...
  async fn test_send_basic_success() {
    ...
    Mock::given(method("POST")).and(path("/index.php/jsonrpc/sms")).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;
  ...
  async fn test_send_multiple_targets() {
    ...
    Mock::given(method("POST")).and(path("/index.php/jsonrpc/sms")).respond_with(ResponseTemplate::new(200)).expect(3).mount(&server).await;

    let addr = server.address();
    let url_str = format!("smseagle://token@{}:{}/11111111111/22222222222/@contact", addr.ip(), addr.port());
    let parsed = crate::utils::parse::ParsedUrl::parse(&url_str).unwrap();
  ...
  async fn test_send_body_only_when_no_title() {
    ...
    let eagle = smseagle_for_mock(&server, &["@user"]);
    let ctx = NotifyContext { title: "".into(), body: "Body Only".into(), ..Default::default() };
    let result = eagle.send(&ctx).await;
    assert!(result.is_ok());
    assert_eq!(result.unwrap(), true);
  }
  ...
  async fn test_send_server_error_returns_false() {
    ...
    Mock::given(method("POST")).and(path("/index.php/jsonrpc/sms")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_send_partial_failure() {
    ...
    Mock::given(method("POST")).and(path("/index.php/jsonrpc/sms")).respond_with(ResponseTemplate::new(500)).mount(&server).await;

    let addr = server.address();
    let url_str = format!("smseagle://token@{}:{}/11111111111/22222222222", addr.ip(), addr.port());
    let parsed = crate::utils::parse::ParsedUrl::parse(&url_str).unwrap();
    let eagle = SmsEagle::from_url(&parsed).unwrap();
  ...
  async fn test_send_secure_url() {
    ...
    let parsed = crate::utils::parse::ParsedUrl::parse("smseagles://token@localhost/@user").unwrap();
    let eagle = SmsEagle::from_url(&parsed).unwrap();
    assert!(eagle.secure);
  }

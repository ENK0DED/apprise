...
impl Xml {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: None,
      setup_url: None,
      protocols: vec!["xml", "xmls"],
      description: "Send an XML notification to any HTTP endpoint.",
...
impl Notify for Xml {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let mut body = format!(
      ...
      xml_escape(&ctx.title),
      xml_escape(&ctx.body),
      ctx.notify_type.as_str()
    );
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["xml://:@/", "xml://", "xmls://", "xml://user@localhost?method=invalid"];
    for url in &urls {
  ...
  async fn test_xml_custom_path() {
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/api/notify")).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;
  ...
  async fn test_xml_with_attachments_base64() {
    ...
    ctx.attachments.push(Attachment { name: "test.txt".to_string(), data: b"hello world".to_vec(), mime_type: "text/plain".to_string() });
    let result = svc.send(&ctx).await.unwrap();
    assert!(result);
  }
}

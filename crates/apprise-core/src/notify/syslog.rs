...
impl Syslog {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: None,
      setup_url: None,
      protocols: vec!["syslog"],
      description: "Send to local syslog.",
...
mod tests {
  ...
  #[test]
  fn test_valid_urls() {
    let valid_urls = vec!["syslog://localhost", "syslog://localhost:514", "syslog://192.168.1.1", "syslog://syslog.example.com:1514"];
    for url in &valid_urls {
      let parsed = ParsedUrl::parse(url);
      assert!(parsed.is_some(), "ParsedUrl::parse failed for: {}", url);
      let parsed = parsed.unwrap();
      assert!(Syslog::from_url(&parsed).is_some(), "Syslog::from_url returned None for valid URL: {}", url,);
    }
  }

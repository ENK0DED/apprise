...
impl Seven {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Seven (seven.io)",
      service_url: Some("https://seven.io"),
      setup_url: None,
      protocols: vec!["seven"],
      description: "Send SMS via seven.io.",
...
impl Notify for Seven {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let resp =
      client.post("https://gateway.seven.io/api/sms").header("User-Agent", APP_ID).header("X-Api-Key", self.apikey.as_str()).json(&payload).send().await?;

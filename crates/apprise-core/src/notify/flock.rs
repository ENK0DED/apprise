...
impl Flock {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Flock",
      service_url: Some("https://flock.com"),
      setup_url: None,
      protocols: vec!["flock"],
      description: "Send via Flock webhooks.",
...
mod tests {
  ...
  fn test_from_url_bare_g_prefix_returns_none() {
    let parsed = crate::utils::parse::ParsedUrl::parse("flock://iiiiiiiiiiiiiiiiiiiiiiii/g:/u:?format=text").unwrap();
    assert!(Flock::from_url(&parsed).is_none());
  }

...
impl NotificationApi {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    if let Some(ch) = url.get("channels") {
      ...
      for c in ch.split(',') {
        ...
        if !c.is_empty() && !valid_channels.contains(&c.as_str()) {
          return None;
        }
      }
    }
    let (client_id, secret) = if let Some(id) = url.get("id") {
      let sec = url.get("secret").map(|s| s.to_string()).or_else(|| url.host.clone()).or_else(|| url.path_parts.first().cloned())?;
      (id.to_string(), sec)
    ...
    // Also add ?to= targets
    let to_targets: Vec<String> = url.get("to").map(|t| t.split(',').map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect()).unwrap_or_default();
    let all_targets: Vec<&str> = targets.iter().copied().chain(to_targets.iter().map(|s| s.as_str())).collect();
    ...
    if !all_targets.is_empty() {
      ...
      for t in &all_targets {
        ...
        if !is_email && !is_phone && !is_named_email {
          ...
          if has_email {
            return None;
          } // duplicate email for same id
          has_email = true;
        } else if is_phone {
          if has_phone {
            return None;
          } // duplicate phone for same id
          has_phone = true;
        }
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://www.notificationapi.com"),
      setup_url: None,
      protocols: vec!["napi", "notificationapi"],
      description: "Send via NotificationAPI.",
...
impl Notify for NotificationApi {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let resp = client
      .post("https://api.notificationapi.com/send")

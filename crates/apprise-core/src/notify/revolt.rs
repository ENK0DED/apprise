...
impl Revolt {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    let bot_token = url.get("bot_token").map(|s| s.to_string()).or_else(|| url.host.clone().filter(|h| !h.is_empty() && h != "_"))?;
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Revolt",
      service_url: Some("https://revolt.chat"),
      setup_url: None,
      protocols: vec!["revolt"],
      description: "Send messages via Revolt.",
...
mod tests {
  ...
  fn test_invalid_urls() {
    ...
    let urls: Vec<&str> = vec!["revolt://", "revolt://:@/", &no_channel, &no_token];
    for url in &urls {

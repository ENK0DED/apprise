...
impl FeiShu {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "FeiShu",
      service_url: Some("https://open.feishu.cn"),
      setup_url: None,
      protocols: vec!["feishu"],
      description: "Send via FeiShu (Lark) bot webhook.",
...
mod tests {
  ...
  #[test]
  fn test_valid_urls() {
    let urls = vec!["feishu://abc123", "feishu://?token=abc123", "feishu://token"];
    for url in &urls {
  ...
  fn test_invalid_urls() {
    let urls = vec!["feishu://", "feishu://:@/", "feishu://%badtoken%"];
    for url in &urls {

...
impl MsTeams {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let webhook_url = if let Some(td) = token_d {
      ...
      if let Some(ref team_name) = team {
        format!("https://{}.webhook.office.com/webhookb2/{}/IncomingWebhook/{}/{}/{}", team_name, token_a, token_b, token_c, td)
      } else {
        format!("https://outlook.office.com/webhook/{}/IncomingWebhook/{}/{}/{}", token_a, token_b, token_c, td)
      }
    } else {
      // v1/v2 format without token_d
      format!("https://outlook.office.com/webhook/{}/IncomingWebhook/{}/{}", token_a, token_b, token_c)
    };
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://teams.microsoft.com"),
      setup_url: None,
      protocols: vec!["msteams"],
      description: "Send via Microsoft Teams incoming webhooks.",
...
mod tests {
  ...
  fn test_webhook_url_with_team_in_user() {
    ...
    let url_str = format!("msteams://myteam@{}/{}/{}/{}", UUID4, "m".repeat(32), UUID4, "extra");
    let parsed = ParsedUrl::parse(&url_str).expect("parse");
    let ms = MsTeams::from_url(&parsed).expect("from_url");
  ...
  fn test_native_url_v1() {
    let url_str = format!("https://outlook.office.com/webhook/{}@{}/IncomingWebhook/{}/{}", UUID4, UUID4, "k".repeat(32), UUID4);
    assert!(from_url(&url_str).is_some(), "Should parse native v1 URL");
  }

  #[test]
  fn test_native_url_v2() {
    let url_str = format!("https://myteam.webhook.office.com/webhookb2/{}@{}/IncomingWebhook/{}/{}", UUID4, UUID4, "m".repeat(32), UUID4);

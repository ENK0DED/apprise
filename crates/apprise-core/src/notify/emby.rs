...
impl Emby {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Emby",
      service_url: Some("https://emby.media"),
      setup_url: None,
      protocols: vec!["emby", "embys"],
      description: "Send notifications to Emby.",
...
impl Notify for Emby {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let auth_header = format!("MediaBrowser Client=\"Apprise\", Device=\"Apprise\", DeviceId=\"apprise\", Version=\"{}\"", env!("CARGO_PKG_VERSION"));
    ...
    let access_token = auth_json["AccessToken"].as_str().ok_or_else(|| NotifyError::Other("No Emby access token".into()))?;

    // Step 2: Get sessions to send to
    let sessions_url = format!("{}/emby/Sessions?api_key={}", base, access_token);
    let sessions_resp = client.get(&sessions_url).header("User-Agent", APP_ID).send().await?;

    let sessions: Vec<serde_json::Value> = sessions_resp.json().await.unwrap_or_default();
    ...
    for session in &sessions {
      if let Some(session_id) = session["Id"].as_str() {
        ...
        let resp = client.post(&msg_url).header("User-Agent", APP_ID).json(&payload).send().await?;
    ...
    // Step 4: Logout
    let _ = client.post(format!("{}/emby/Sessions/Logout?api_key={}", base, access_token)).header("User-Agent", APP_ID).send().await;
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["emby://", "embys://", "emby://localhost", "emby://:@/"];
    for url in &urls {
  ...
  async fn test_send_success() {
    ...
    Mock::given(method("POST")).and(path_regex("/emby/Sessions/.*/Message")).respond_with(ResponseTemplate::new(200)).expect(2).mount(&server).await;

    // Mock logout
    Mock::given(method("POST")).and(path_regex("/emby/Sessions/Logout")).respond_with(ResponseTemplate::new(200)).mount(&server).await;
  ...
  async fn test_send_auth_failure() {
    ...
    // Auth returns 401
    Mock::given(method("POST")).and(path_regex("/emby/Users/AuthenticateByName")).respond_with(ResponseTemplate::new(401)).expect(1).mount(&server).await;
  ...
  async fn test_send_server_error() {
    ...
    // Auth returns 500
    Mock::given(method("POST")).and(path_regex("/emby/Users/AuthenticateByName")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_send_message_failure() {
    ...
    // Message send fails
    Mock::given(method("POST")).and(path_regex("/emby/Sessions/.*/Message")).respond_with(ResponseTemplate::new(500)).mount(&server).await;
  ...
  #[test]
  fn test_valid_urls() {
    let urls = vec!["emby://l2g:pass@localhost", "embys://l2g:password@localhost"];
    for url in &urls {

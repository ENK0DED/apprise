...
impl Notify for Discord {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let mut url = format!("{}/api/webhooks/{}/{}", base, self.webhook_id, self.webhook_token);
    ...
    let resp = client.post(&url).header("User-Agent", APP_ID).header("Content-Type", "application/json").json(&payload).send().await?;
...
mod tests {
  ...
  async fn test_empty_title_omits_title_in_embed() {
    ...
    Mock::given(method("POST")).and(path(webhook_path())).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;
  ...
  async fn test_footer_logo_in_embed() {
    ...
    let url = format!("discord://{}/{}?footer=Yes&footer_logo=http://example.com/logo.png", WH_ID, WH_TOKEN);
    let d = discord_from_url_with_mock(&server, &url);
    let result = d.send(&ctx("title", "body")).await;
    assert!(result.is_ok());
    assert!(result.unwrap());
  ...
  async fn test_http_500_returns_error() {
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path(webhook_path())).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_http_204_no_content_succeeds() {
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path(webhook_path())).respond_with(ResponseTemplate::new(204)).expect(1).mount(&server).await;
  ...
  async fn test_attachment_sends_two_requests() {
    ...
    Mock::given(method("POST")).and(path(webhook_path())).respond_with(ResponseTemplate::new(200)).expect(2).mount(&server).await;

    let url = format!("discord://{}/{}", WH_ID, WH_TOKEN);
    let d = discord_from_url_with_mock(&server, &url);

    let mut c = ctx("title", "body");
    c.attachments.push(Attachment { name: "test.gif".to_string(), data: b"GIF89a".to_vec(), mime_type: "image/gif".to_string() });
  ...
  async fn test_multiple_attachments_send_multiple_requests() {
    ...
    Mock::given(method("POST")).and(path(webhook_path())).respond_with(ResponseTemplate::new(200)).expect(3).mount(&server).await;

    let url = format!("discord://{}/{}", WH_ID, WH_TOKEN);
    let d = discord_from_url_with_mock(&server, &url);

    let mut c = ctx("title", "body");
    c.attachments.push(Attachment { name: "a.gif".to_string(), data: b"GIF89a".to_vec(), mime_type: "image/gif".to_string() });
    c.attachments.push(Attachment { name: "b.png".to_string(), data: b"PNG".to_vec(), mime_type: "image/png".to_string() });
  ...
  async fn test_attachment_upload_after_json_failure_returns_error() {
    ...
    let mut c = ctx("title", "body");
    c.attachments.push(Attachment { name: "file.gif".to_string(), data: b"GIF89a".to_vec(), mime_type: "image/gif".to_string() });

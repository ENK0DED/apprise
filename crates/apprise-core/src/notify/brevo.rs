...
impl Brevo {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Brevo (Sendinblue)",
      service_url: Some("https://brevo.com"),
      setup_url: None,
      protocols: vec!["brevo"],
      description: "Send email via Brevo (formerly Sendinblue).",
...
impl Notify for Brevo {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let resp =
      client.post("https://api.brevo.com/v3/smtp/email").header("User-Agent", APP_ID).header("api-key", self.apikey.as_str()).json(&payload).send().await?;
...
mod tests {
  ...
  fn test_from_url_struct_fields() {
    let parsed = crate::utils::parse::ParsedUrl::parse("brevo://myapikey:sender@example.com/recipient@example.com").unwrap();
  ...
  fn test_from_url_no_target_uses_from() {
    let parsed = crate::utils::parse::ParsedUrl::parse("brevo://abcd:user@example.com?format=text").unwrap();
    let obj = Brevo::from_url(&parsed).unwrap();
  ...
  fn test_from_url_multiple_targets() {
    let parsed = crate::utils::parse::ParsedUrl::parse("brevo://abcd:user@example.com/one@test.com?to=two@test.com").unwrap();
    let obj = Brevo::from_url(&parsed).unwrap();
  ...
  fn test_invalid_apikey_special_chars() {
    let parsed = crate::utils::parse::ParsedUrl::parse("brevo://invalid-api-key+*-d:user@example.com").unwrap();
    assert!(Brevo::from_url(&parsed).is_none());
  }

  #[test]
  fn test_invalid_reply_to() {
    let parsed = crate::utils::parse::ParsedUrl::parse("brevo://abcd:user@example.com/newuser@example.com?reply=%20!").unwrap();
    assert!(Brevo::from_url(&parsed).is_none());
  }

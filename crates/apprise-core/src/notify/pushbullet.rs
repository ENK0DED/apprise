...
impl Pushbullet {
  ...
  fn set_target(payload: &mut serde_json::Value, t: &str) {
    if t.contains('@') {
      ...
      payload["channel_tag"] = json!(stripped);
    } else {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://pushbullet.com"),
      setup_url: None,
      protocols: vec!["pbul"],
...
impl Notify for Pushbullet {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let targets: Vec<Option<&String>> = if self.targets.is_empty() { vec![None] } else { self.targets.iter().map(Some).collect() };
    ...
    for attachment in &ctx.attachments {
      ...
      let part = reqwest::multipart::Part::bytes(attachment.data.clone())
        ...
        .unwrap_or_else(|_| reqwest::multipart::Part::bytes(attachment.data.clone()).file_name(attachment.name.clone()));
      let form = reqwest::multipart::Form::new().part("file", part);
      let resp = client.post(&upload_url).header("User-Agent", APP_ID).multipart(form).send().await?;

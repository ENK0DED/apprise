...
impl Chanify {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://chanify.net"),
      setup_url: None,
      protocols: vec!["chanify"],
      description: "Send notifications via Chanify.",
...
mod tests {
  ...
  #[test]
  fn test_valid_urls() {
    let urls = vec!["chanify://abc123", "chanify://?token=abc123", "chanify://token"];
    for url in &urls {
  ...
  fn test_invalid_urls() {
    let urls = vec!["chanify://", "chanify://:@/", "chanify://%badtoken%"];
    for url in &urls {
  ...
  fn test_from_url_struct_fields() {
    let parsed = crate::utils::parse::ParsedUrl::parse("chanify://my-secret-token").unwrap();
  ...
  fn test_from_url_token_query_param() {
    let parsed = crate::utils::parse::ParsedUrl::parse("chanify://?token=abc123").unwrap();
    let obj = Chanify::from_url(&parsed).unwrap();

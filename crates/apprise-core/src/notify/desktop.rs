...
impl Desktop {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Desktop Notification",
      service_url: None,
      setup_url: None,
      protocols: vec!["dbus", "kde", "qt", "glib", "gnome"],
...
impl Notify for Desktop {
  ...
  async fn send(&self, _ctx: &NotifyContext) -> Result<bool, NotifyError> {
    #[cfg(feature = "desktop")]
    {
      use notify_rust::Notification;
      Notification::new().appname(&self.app_name).summary(&ctx.title).body(&ctx.body).show().map_err(|e| NotifyError::Other(e.to_string()))?;
      return Ok(true);
    }
...
mod tests {
  ...
  #[test]
  fn test_valid_urls() {
    let valid_urls = vec!["dbus://MyApp", "kde://MyApp", "gnome://MyApp", "qt://MyApp", "glib://MyApp"];
    for url in &valid_urls {
      let parsed = ParsedUrl::parse(url);
      assert!(parsed.is_some(), "ParsedUrl::parse failed for: {}", url);
      let parsed = parsed.unwrap();
      assert!(Desktop::from_url(&parsed).is_some(), "Desktop::from_url returned None for valid URL: {}", url,);
    }
  }
  ...
  fn test_desktop_default_app_name() {
    ...
    let parsed = ParsedUrl::parse("dbus://").unwrap_or_else(|| {
      ...
      ParsedUrl {
        ...
        path: String::new(),
        path_parts: vec![],
        qsd: std::collections::HashMap::new(),
        raw: "dbus://".to_string(),
      }
    });
    let d = Desktop::from_url(&parsed).unwrap();

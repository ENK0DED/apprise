...
mod tests {
  ...
  fn nextcloud_for_mock(server: &MockServer, user: &str, pass: &str, targets: &[&str]) -> Nextcloud {
    ...
    let url_str = format!("ncloud://{}:{}@{}:{}{}", user, pass, addr.ip(), addr.port(), target_path);
  ...
  async fn test_send_multiple_targets() {
    ...
    for target in &["user1", "user2", "user3"] {
      ...
      Mock::given(method("POST")).and(path(p)).respond_with(ResponseTemplate::new(200)).expect(1).named(&format!("target {}", target)).mount(&server).await;
    }

...
impl FreeMobile {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let user = url.user.clone().or_else(|| url.get("user").map(|s| s.to_string()))?;
    let password = url.password.clone().or_else(|| url.host.clone().filter(|h| !h.is_empty())).or_else(|| url.get("pass").map(|s| s.to_string()))?;
    Some(Self { user, password, verify_certificate: url.verify_certificate(), tags: url.tags() })
  }
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Free Mobile",
      service_url: Some("https://mobile.free.fr"),
      setup_url: None,
      protocols: vec!["freemobile"],
      description: "Send SMS via Free Mobile (France).",
...
mod tests {
  ...
  #[test]
  fn test_valid_urls() {
    let urls = vec!["freemobile://user@password", "freemobile://?user=user&pass=password"];
    for url in &urls {
  ...
  fn test_invalid_urls() {
    let urls = vec!["freemobile://", "freemobile://:@/"];
    for url in &urls {

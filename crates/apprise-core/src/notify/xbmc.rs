...
impl Xbmc {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://kodi.tv"),
      setup_url: None,
      protocols: vec!["xbmc", "kodi", "kodis"],
      description: "Send notifications to Kodi/XBMC.",
...
impl Notify for Xbmc {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let payload =
      json!({ "jsonrpc": "2.0", "method": "GUI.ShowNotification", "params": { "title": ctx.title, "message": ctx.body, "displaytime": 5000 }, "id": 1 });
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["kodi://", "kodis://", "kodi://:@/", "xbmc://", "xbmc://:@/"];
    for url in &urls {
  ...
  async fn test_send_basic_success() {
    ...
    Mock::given(method("POST")).and(path("/jsonrpc")).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;
  ...
  async fn test_send_server_error_returns_false() {
    ...
    Mock::given(method("POST")).and(path("/jsonrpc")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_send_bizarre_status_code() {
    ...
    Mock::given(method("POST")).and(path("/jsonrpc")).respond_with(ResponseTemplate::new(418)).expect(1).mount(&server).await;
  ...
  async fn test_send_includes_user_agent() {
    ...
    Mock::given(method("POST")).and(path("/jsonrpc")).and(header("User-Agent", APP_ID)).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;

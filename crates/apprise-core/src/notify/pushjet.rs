...
impl PushJet {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://pushjet.io"),
      setup_url: None,
      protocols: vec!["pjet", "pjets"],
      description: "Send push notifications via Pushjet.",
...
mod tests {
  ...
  async fn test_send_success() {
    ...
    Mock::given(method("POST")).and(path(format!("/message"))).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;
  ...
  async fn test_send_server_error() {
    ...
    Mock::given(method("POST")).and(path("/message")).respond_with(ResponseTemplate::new(500).set_body_string("Error")).expect(1).mount(&server).await;

...
impl PushPlus {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    let token = url.get("token").map(|s| s.to_string()).or_else(|| url.host.clone().filter(|h| !h.is_empty() && h != "www.pushplus.plus"))?;
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://www.pushplus.plus"),
      setup_url: None,
      protocols: vec!["pushplus"],
      description: "Send notifications via PushPlus.",
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["pushplus://", "pushplus://invalid!"];
    for url in &urls {
  ...
  fn test_from_url_https_format() {
    let parsed = ParsedUrl::parse("https://www.pushplus.plus/send?token=abc123def456ghi789jkl012mno345pq").unwrap();
    let p = PushPlus::from_url(&parsed).unwrap();
    assert_eq!(p.token, "abc123def456ghi789jkl012mno345pq");
  }

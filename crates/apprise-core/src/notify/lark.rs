...
impl Lark {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    let token = if url.schema == "https" || url.schema == "http" {
      ...
      url.host.clone().filter(|h| !h.is_empty() && h != "_").or_else(|| url.get("token").map(|s| s.to_string()))
    }?;
    if token.is_empty() {
      return None;
    }
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Lark",
      service_url: Some("https://larksuite.com"),
      setup_url: None,
      protocols: vec!["lark"],
      description: "Send via Lark (Feishu international) webhooks.",
...
mod tests {
  ...
  #[test]
  fn test_valid_urls() {
    let urls = vec!["lark://abcd-1234", "lark://?token=abcd-1234", "https://open.larksuite.com/open-apis/bot/v2/hook/abcd-1234"];
    for url in &urls {

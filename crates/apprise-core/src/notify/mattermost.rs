...
impl Mattermost {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let (mode, mut channels) = if is_bot_mode {
      ...
      let token = url.path_parts.first().cloned().or_else(|| url.user.clone())?;
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://mattermost.com"),
      setup_url: None,
      protocols: vec!["mmost", "mmosts"],
      description: "Send via Mattermost webhooks or bot API.",
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    match &self.mode {
      MattermostMode::Webhook { webhook_path } => {
        ...
        let targets: Vec<Option<&String>> = if self.channels.is_empty() { vec![None] } else { self.channels.iter().map(Some).collect() };
        let mut all_ok = true;
        for ch in targets {
      ...
      MattermostMode::Bot { access_token } => {
        ...
        for channel in &self.channels {
          ...
          let resp = client.post(&url).header("User-Agent", APP_ID).header("Authorization", format!("Bearer {}", access_token)).json(&payload).send().await?;
          if !resp.status().is_success() {
            all_ok = false;
          }
        }
...
mod tests {
  ...
  async fn test_webhook_post_json_payload() {
    ...
    Mock::given(method("POST")).and(path("/hooks/token")).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;
  ...
  async fn test_webhook_channel_targeting() {
    ...
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/hooks/token")).respond_with(ResponseTemplate::new(200)).expect(2).mount(&server).await;
  ...
  async fn test_webhook_http_500_returns_false() {
    ...
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/hooks/token")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_webhook_bizarre_status_code_returns_false() {
    ...
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/hooks/token")).respond_with(ResponseTemplate::new(599)).expect(1).mount(&server).await;
  ...
  async fn test_webhook_partial_failure_returns_false() {
    ...
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/hooks/token")).respond_with(ResponseTemplate::new(500)).mount(&server).await;
  ...
  fn test_https_hook_url_parses() {
    ...
    let parsed = ParsedUrl::parse("https://mattermost.example.com/hooks/3ccdd113474722377935511fc85d3dd4").unwrap();
    let mm = Mattermost::from_url(&parsed).unwrap();
    assert!(mm.secure);
  ...
  async fn test_bot_post_with_bearer_token() {
    ...
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/api/v4/posts")).respond_with(ResponseTemplate::new(201)).expect(1).mount(&server).await;
  ...
  async fn test_bot_multiple_channels() {
    ...
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/api/v4/posts")).respond_with(ResponseTemplate::new(201)).expect(2).mount(&server).await;
  ...
  async fn test_bot_no_channels_returns_true_no_requests() {
    ...
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/api/v4/posts")).respond_with(ResponseTemplate::new(201)).expect(0).mount(&server).await;
  ...
  async fn test_bot_http_401_returns_false() {
    ...
    let server = MockServer::start().await;
    Mock::given(method("POST")).and(path("/api/v4/posts")).respond_with(ResponseTemplate::new(401)).expect(1).mount(&server).await;

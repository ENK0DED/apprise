...
impl Kavenegar {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    if apikey.is_empty() || !apikey.chars().all(|c| c.is_ascii_alphanumeric()) {
      return None;
    }
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://kavenegar.com"),
      setup_url: None,
      protocols: vec!["kavenegar"],
      description: "Send SMS via Kavenegar.",
...
impl Notify for Kavenegar {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let mut url =
      format!("https://api.kavenegar.com/v1/{}/sms/send.json?receptor={}&message={}", self.apikey, urlencoding::encode(&receptor), urlencoding::encode(&msg));
    if let Some(ref s) = self.sender {
      url.push_str(&format!("&sender={}", urlencoding::encode(s)));
    }

...
impl Qq {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "QQ (Qmsg)",
      service_url: Some("https://qmsg.zendee.cn"),
      setup_url: None,
      protocols: vec!["qq"],
      description: "Send notifications via QQ Qmsg.",
...
mod tests {
  ...
  fn test_from_url_https_format() {
    let parsed = ParsedUrl::parse("https://qmsg.zendee.cn/send/abc123def456ghi789jkl012mno345pq").unwrap();
    let q = Qq::from_url(&parsed).unwrap();
    assert_eq!(q.token, "abc123def456ghi789jkl012mno345pq");
  }

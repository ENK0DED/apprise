...
impl Xmpp {
    pub fn from_url(url: &ParsedUrl) -> Option<Self> {
        ...
        if all_targets.is_empty() { return None; }
...
async fn xmpp_session(
    ...
    host: &str, jid: &str, password: &str,
    targets: &[String], msg: &str,
    ...
    xmpp_write(writer, &format!(
        "<auth xmlns='urn:ietf:params:xml:ns:xmpp-sasl' mechanism='PLAIN'>{}</auth>", auth_b64
    )).await?;
    let resp = xmpp_read(reader).await?;
        ...
        match self.secure_mode {
            XmppSecureMode::Tls => {
                ...
                let tls_stream = connector.connect(domain, tcp).await
                    .map_err(|e| NotifyError::Other(format!("XMPP TLS handshake failed: {}", e)))?;
            ...
            XmppSecureMode::StartTls => {
                ...
                let mut buf = vec![0u8; 8192];
                let n = tokio::time::timeout(tokio::time::Duration::from_secs(10), tcp.read(&mut buf)).await
                ...
                // Send STARTTLS
                tcp.write_all(b"<starttls xmlns='urn:ietf:params:xml:ns:xmpp-tls'/>").await
                    .map_err(|e| NotifyError::Other(e.to_string()))?;
                let n = tokio::time::timeout(tokio::time::Duration::from_secs(10), tcp.read(&mut buf)).await
                    .map_err(|_| NotifyError::Other("XMPP timeout".into()))?
                    .map_err(|e| NotifyError::Other(e.to_string()))?;
                ...
                let domain = self.server_name()?;
                let tls_stream = connector.connect(domain, tcp).await
                    .map_err(|e| NotifyError::Other(format!("XMPP STARTTLS upgrade failed: {}", e)))?;
                let (mut reader, mut writer) = tokio::io::split(tls_stream);
                xmpp_session(&mut reader, &mut writer, &self.host, &self.jid, &self.password, &self.targets, &msg).await
            }
        }
...
mod tests {
    ...
    fn test_valid_urls() {
        let valid_urls = vec![
            "xmpp://user:pass@localhost/target@example.com",
            "xmpps://user:pass@localhost/target@example.com",
        ];
        for url in &valid_urls {
            ...
            assert!(
                Xmpp::from_url(&parsed).is_some(),
                "Xmpp::from_url returned None for valid URL: {}",
                url,
            );
        }
    }
    ...
    fn test_invalid_urls() {
        ...
        for url in &invalid_urls {
            ...
            assert!(
                result.is_none(),
                "Xmpp::from_url should return None for: {}",
                url,
            );
        }
    }

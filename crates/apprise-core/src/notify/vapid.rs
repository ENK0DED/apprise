...
impl Vapid {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let subscriber = if let Some(ref user) = url.user {
      ...
      if host.contains('@') { format!("mailto:{}", host) } else { host.clone() }
    } else {
      return None;
    };
...
impl Notify for Vapid {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    for endpoint in &self.endpoints {
      ...
      let resp =
        client.post(endpoint).header("User-Agent", APP_ID).header("Content-Type", "application/json").header("TTL", "86400").body(body.clone()).send().await?;
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["vapid://", "vapid://:@/", "vapid://invalid-subscriber"];
    for url in &urls {
  ...
  async fn test_send_success() {
    ...
    Mock::given(method("POST")).respond_with(ResponseTemplate::new(201)).expect(1).mount(&server).await;

    let v = Vapid { subscriber: "mailto:user@example.com".into(), endpoints: vec![format!("{}/push", server.uri())], verify_certificate: false, tags: vec![] };

    let ctx = NotifyContext { title: "Test".into(), body: "Hello".into(), ..Default::default() };
  ...
  async fn test_send_multiple_endpoints() {
    ...
    Mock::given(method("POST")).respond_with(ResponseTemplate::new(201)).expect(2).mount(&server).await;

    let v = Vapid {
      subscriber: "mailto:user@example.com".into(),
      endpoints: vec![format!("{}/push1", server.uri()), format!("{}/push2", server.uri())],

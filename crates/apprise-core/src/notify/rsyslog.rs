...
impl RSyslog {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: None,
      setup_url: None,
      protocols: vec!["rsyslog"],
      description: "Send via RSyslog (UDP).",
...
mod tests {
  ...
  #[test]
  fn test_valid_urls() {
    let valid_urls = vec!["rsyslog://localhost", "rsyslog://localhost:514", "rsyslog://192.168.1.1", "rsyslog://syslog.example.com:1514"];
    for url in &valid_urls {
      let parsed = ParsedUrl::parse(url);
      assert!(parsed.is_some(), "ParsedUrl::parse failed for: {}", url);
      let parsed = parsed.unwrap();
      assert!(RSyslog::from_url(&parsed).is_some(), "RSyslog::from_url returned None for valid URL: {}", url,);
    }
  }

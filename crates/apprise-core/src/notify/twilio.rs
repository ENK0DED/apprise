...
impl Twilio {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    if from_phone.contains(':') || from_phone == "_" {
      return None;
    }
    ...
    if let Some(method) = url.get("method") {
      ...
      if method.to_lowercase() == "call" && from_phone.starts_with("w:") {
        return None;
      }
    }
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Twilio",
      service_url: Some("https://twilio.com"),
      setup_url: None,
      protocols: vec!["twilio"],
...
mod tests {
  ...
  fn test_valid_urls() {
    let urls = vec![
      ...
      format!("twilio://AC{}:{}@{}/123/{}/abcd/w:{}", "a".repeat(32), "b".repeat(32), "3".repeat(11), "9".repeat(15), 8u64 * 11),
      ...
      // Query param with to=
      format!("twilio://_?sid=AC{}&token={}&from={}&to={}", "a".repeat(32), "b".repeat(32), "5".repeat(11), "7".repeat(13)),
      // Whatsapp target
      format!("twilio://_?sid=AC{}&token={}&from={}&to=w:{}", "a".repeat(32), "b".repeat(32), "5".repeat(11), "6".repeat(11)),
    ];
    for url in &urls {
  ...
  fn test_multiple_targets() {
    let url_str = format!("twilio://AC{}:{}@{}/{}/{}", "a".repeat(32), "b".repeat(32), "3".repeat(11), "4".repeat(11), "5".repeat(11));
    let parsed = ParsedUrl::parse(&url_str).unwrap();
    let tw = Twilio::from_url(&parsed).unwrap();

...
impl BurstSms {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://burstsms.com.au"),
      setup_url: None,
      protocols: vec!["burstsms"],
      description: "Send SMS via Burst SMS.",
...
impl Notify for BurstSms {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    for target in &self.targets {
      ...
      let resp = client
        .post("https://api.transmitsms.com/send-sms.json")
        .header("User-Agent", APP_ID)
        .basic_auth(&self.apikey, Some(&self.api_secret))
        .form(&params)
        .send()
        .await?;
...
mod tests {
  ...
  fn test_from_url_struct_fields() {
    let parsed = crate::utils::parse::ParsedUrl::parse("burstsms://mykey123:mysecretvalue1234@15551233456/15555555555").unwrap();
  ...
  fn test_from_url_query_params() {
    let parsed = crate::utils::parse::ParsedUrl::parse("burstsms://_?key=testkey1&secret=testsecret123456&from=55555555555&to=66666666666").unwrap();
    let obj = BurstSms::from_url(&parsed).unwrap();
  ...
  fn test_source_alias() {
    let parsed = crate::utils::parse::ParsedUrl::parse("burstsms://_?key=aaaaaaaa&secret=bbbbbbbbbbbbbbbb&source=55555555555&to=66666666666").unwrap();
    let obj = BurstSms::from_url(&parsed).unwrap();
    assert_eq!(obj.from_phone, "55555555555");
  }

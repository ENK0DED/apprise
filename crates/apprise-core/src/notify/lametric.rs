...
impl LaMetric {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let app_id = url.get("app").map(|s| s.to_string()).or_else(|| {
      if let Some(ref h) = host {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://lametric.com"),
      setup_url: None,
      protocols: vec!["lametric", "lametrics"],
      description: "Send notifications to LaMetric devices.",
...
impl Notify for LaMetric {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let url = if let Some(ref aid) = self.app_id {
      format!("https://developer.lametric.com/api/v1/dev/widget/update/com.lametric.{}/1", aid)
    } else {
      "https://developer.lametric.com/api/v1/dev/widget/update/1".to_string()
    };
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec![
      ...
      // Invalid app_ver
      format!("lametrics://{}==@com.lametric.941c51dff3135bd87aa72db9d855dd50/?app_ver=invalid", "A".repeat(88)),
    ];
    for url in &urls {
  ...
  fn test_valid_urls() {
    let urls = vec![
      ...
      // Cloud mode
      format!("lametrics://{}==@com.lametric.941c51dff3135bd87aa72db9d855dd50/?mode=cloud&app_ver=2", "A".repeat(88)),
      // Cloud mode via query params
      format!("lametric://?app=com.lametric.941c51dff3135bd87aa72db9d855dd50&token={}==&mode=cloud", "B".repeat(88)),
      // Cloud mode with sound, icon_type, priority
      format!("lametrics://{}==@abcd/?mode=cloud&sound=knock&icon_type=info&priority=critical&cycles=10", "C".repeat(88)),
  ...
  fn test_native_url() {
    ...
    let url = format!("https://developer.lametric.com/api/v1/dev/widget/update/com.lametric.ABCD123/1?token={}==", "D".repeat(88));
  ...
  fn test_from_url_cloud_mode_fields() {
    ...
    let obj = parse_lametric(&format!("lametrics://{}@com.lametric.941c51dff3135bd87aa72db9d855dd50/?mode=cloud&app_ver=2", token)).unwrap();

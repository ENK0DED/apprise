...
impl BulkSms {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://bulksms.com"),
      setup_url: None,
      protocols: vec!["bulksms"],
      description: "Send SMS via BulkSMS.",
...
mod tests {
  ...
  fn test_from_url_struct_fields() {
    let parsed = crate::utils::parse::ParsedUrl::parse("bulksms://myuser:mypass@+15551231234/+15555555555").unwrap();
  ...
  fn test_from_url_query_params() {
    let parsed = crate::utils::parse::ParsedUrl::parse("bulksms://_?user=testuser&password=testpass&from=55555555555&to=66666666666").unwrap();
    let obj = BulkSms::from_url(&parsed).unwrap();
  ...
  fn test_route_validation() {
    ...
    for route in &["economy", "standard", "premium"] {
      let url = format!("bulksms://aaaaaaaaaa:bbbbbbbbbb@admin?route={}", route);
    ...
    // Invalid route
    let parsed = crate::utils::parse::ParsedUrl::parse("bulksms://aaaaaaaaaa:bbbbbbbbbb@admin?route=invalid").unwrap();

...
impl Splunk {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let apikey = url.host.clone().filter(|h| !h.is_empty()).or_else(|| url.get("apikey").map(|s| s.to_string()))?;

    // Reject invalid tokens (percent-encoded bad values)
    let decoded_api = urlencoding::decode(&apikey).unwrap_or_default().into_owned();
    if decoded_api.trim().is_empty() || decoded_api.contains('%') {
      return None;
    }
    ...
    if has_userinfo && url.user.as_ref().is_none_or(|u| u.trim().is_empty()) {
    ...
    if let Some(action) = url.get("action") {
      match action.to_lowercase().as_str() {
        "recovery" | "resolve" | "r" | "acknowledgement" | "ack" | "a" | "critical" | "crit" | "c" | "warning" | "warn" | "w" | "info" | "i" | "" => {}
        _ => return None,
      }
    }

    // Validate message type remapping (:field=value)
    let valid_types = ["recovery", "resolve", "acknowledgement", "ack", "critical", "crit", "warning", "warn", "info"];
    for (k, v) in &url.qsd {
...
impl Notify for Splunk {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let url = format!("https://alert.victorops.com/integrations/generic/20131114/alert/{}/{}", self.apikey, self.routing_key);
...
mod tests {
  ...
  fn test_from_url_user_at_host() {
    let parsed = crate::utils::parse::ParsedUrl::parse("splunk://myroute@myapikey").unwrap();
  ...
  fn test_from_url_query_params() {
    let parsed = crate::utils::parse::ParsedUrl::parse("splunk://?apikey=abc123&routing_key=db").unwrap();
    let splunk = Splunk::from_url(&parsed).unwrap();
  ...
  fn test_from_url_route_alias() {
    let parsed = crate::utils::parse::ParsedUrl::parse("splunk://?apikey=abc123&route=myroute").unwrap();
    let splunk = Splunk::from_url(&parsed).unwrap();
    assert_eq!(splunk.routing_key, "myroute");
  }
  ...
  fn test_from_url_with_entity_id_in_path() {
    let parsed = crate::utils::parse::ParsedUrl::parse("splunk://route@abc123/entity_id").unwrap();
    let splunk = Splunk::from_url(&parsed).unwrap();
    assert_eq!(splunk.apikey, "abc123");
  ...
  fn test_from_url_legacy_victorops_url() {
    let parsed = crate::utils::parse::ParsedUrl::parse("https://alert.victorops.com/integrations/generic/20131114/alert/myapi/myroute").unwrap();
    let splunk = Splunk::from_url(&parsed).unwrap();
  ...
  fn test_action_validation() {
    let valid_actions = vec!["recovery", "resolve", "r", "acknowledgement", "ack", "critical", "crit", "warning", "warn", "info", "i"];

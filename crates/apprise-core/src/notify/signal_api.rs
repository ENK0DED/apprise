...
impl SignalApi {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let source = url.get("from").or_else(|| url.get("source")).map(|s| s.to_string()).or_else(|| url.path_parts.first().cloned())?;
    if source.is_empty() {
      return None;
    }
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://signal.org"),
      setup_url: None,
      protocols: vec!["signal", "signals"],
      description: "Send Signal messages via signal-cli REST API.",
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    if !ctx.attachments.is_empty() {
      payload["base64_attachments"] =
        json!(ctx.attachments.iter().map(|att| { base64::engine::general_purpose::STANDARD.encode(&att.data) }).collect::<Vec<_>>());
    }
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["signal://", "signal://:@/", "signal://localhost", "signal://localhost/123"];
    for url in &urls {
  ...
  async fn test_send_basic_success() {
    ...
    Mock::given(method("POST")).and(path("/v2/send")).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;
  ...
  async fn test_send_with_attachments() {
    ...
    let ctx = NotifyContext {
      ...
      attachments: vec![Attachment { name: "test.gif".into(), data: b"fake image data".to_vec(), mime_type: "image/gif".into() }],
      ..Default::default()
    };
    let result = sig.send(&ctx).await;
    assert!(result.is_ok());
  ...
  async fn test_send_no_attachments_no_base64_field() {
    ...
    let sig = signal_for_mock(&server, &source, &[&target]);
    let ctx = NotifyContext { body: "no attachments".into(), ..Default::default() };
    let result = sig.send(&ctx).await;
    assert!(result.is_ok());
  }
  ...
  async fn test_send_server_error() {
    ...
    Mock::given(method("POST")).and(path("/v2/send")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_send_non_standard_error() {
    ...
    Mock::given(method("POST")).and(path("/v2/send")).respond_with(ResponseTemplate::new(418)).expect(1).mount(&server).await;
  ...
  async fn test_send_with_basic_auth() {
    ...
    let url_str = format!("signal://myuser:mypass@{}:{}/{}/{}", addr.ip(), addr.port(), source, target);
    let parsed = crate::utils::parse::ParsedUrl::parse(&url_str).unwrap();
    ...
    let ctx = NotifyContext { body: "auth test".into(), ..Default::default() };
    let result = sig.send(&ctx).await;
    assert!(result.is_ok());
    assert!(result.unwrap());
  }
  ...
  fn test_secure_vs_insecure() {
    ...
    let parsed = crate::utils::parse::ParsedUrl::parse(&format!("signal://localhost/{}/{}", source, target)).unwrap();
    let sig = SignalApi::from_url(&parsed).unwrap();
    assert!(!sig.secure);

    let parsed = crate::utils::parse::ParsedUrl::parse(&format!("signals://localhost/{}/{}", source, target)).unwrap();
    let sig = SignalApi::from_url(&parsed).unwrap();
    assert!(sig.secure);
  }

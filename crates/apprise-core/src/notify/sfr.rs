...
impl Sfr {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "SFR",
      service_url: Some("https://www.sfr.fr"),
      setup_url: None,
      protocols: vec!["sfr"],
      description: "Send SMS via SFR (France).",
...
impl Notify for Sfr {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    for target in &self.targets {
      ...
      let resp = client
        .post("https://www.dmc.sfr-sh.fr/DmcWS/1.5.8/JsonService/MessagesUnitairesWS/addSingleCall")
...
mod tests {
  ...
  fn test_from_url_fields() {
    let url_str = format!("sfr://srv:pwd@{}/{}", "1".repeat(8), "0".repeat(10),);
  ...
  fn test_from_url_multiple_targets() {
    let url_str = format!("sfr://444444:other_password@{}/{}/{}", "1".repeat(8), "6".repeat(10), "8".repeat(10),);
    let parsed = crate::utils::parse::ParsedUrl::parse(&url_str).unwrap();
    let sfr = Sfr::from_url(&parsed).unwrap();

...
impl Matrix {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let access_token = url
      ...
      .or_else(|| {
        ...
        if (url.schema == "https" || url.schema == "http") && url.path_parts.len() >= 2 { url.path_parts.last().cloned() } else { None }
      })
      .or_else(|| {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Matrix",
      service_url: Some("https://matrix.org"),
      setup_url: None,
      protocols: vec!["matrix", "matrixs"],
      description: "Send via Matrix room messages.",
...
impl Notify for Matrix {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    if self.rooms.is_empty() {
      return Err(NotifyError::MissingParam("room_id".into()));
    }
      ...
      for att in &ctx.attachments {
        ...
        if let Ok(upload_json) = upload_resp.json::<serde_json::Value>().await {
          if let Some(mxc_uri) = upload_json["content_uri"].as_str() {
            ...
            let file_url =
              format!("{}://{}{}/_matrix/client/r0/rooms/{}/send/m.room.message/{}", schema, self.host, port_str, urlencoding::encode(room), file_txn);
            ...
            let _ = client.put(&file_url).header("Authorization", format!("Bearer {}", self.access_token)).json(&file_payload).send().await;
          }
        }
      }
    }
...
mod tests {
  ...
  async fn test_send_message_to_single_room() {
    ...
    Mock::given(method("PUT"))
      ...
      .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"event_id": "$abc123"})))
      .expect(1)
      .mount(&server)
      .await;
  ...
  async fn test_send_to_multiple_rooms() {
    ...
    Mock::given(method("PUT"))
      ...
      .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"event_id": "$ev"})))
      .expect(3)
      .mount(&server)
      .await;

    let m = matrix_with_token(&server, "tok", vec!["!room1:localhost", "!room2:localhost", "!room3:localhost"]);
  ...
  async fn test_send_http_500_returns_false() {
    ...
    Mock::given(method("PUT"))
      .and(path_regex(r"/_matrix/client/r0/rooms/.+/send/m\.room\.message/.+"))
      .respond_with(ResponseTemplate::new(500).set_body_json(serde_json::json!({"errcode": "M_UNKNOWN"})))
      .expect(1)
      .mount(&server)
      .await;
  ...
  async fn test_partial_failure_returns_false() {
    ...
    let m = matrix_with_token(&server, "tok", vec!["!room1:host", "!room2:host"]);
    let result = m.send(&ctx("", "test")).await;
    assert!(result.is_ok());
  ...
  async fn test_attachment_upload_and_file_message() {
    ...
    Mock::given(method("PUT"))
      .and(path_regex(r"/_matrix/client/r0/rooms/.+/send/m\.room\.message/.+"))
      .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"event_id": "$msg"})))
      .mount(&server)
      .await;
    ...
    c.attachments.push(crate::notify::Attachment { name: "photo.jpg".to_string(), data: b"JFIF".to_vec(), mime_type: "image/jpeg".to_string() });
  ...
  async fn test_non_image_attachment_uses_m_file() {
    ...
    c.attachments.push(crate::notify::Attachment { name: "document.pdf".to_string(), data: b"%PDF".to_vec(), mime_type: "application/pdf".to_string() });
  ...
  async fn test_multiple_attachments_per_room() {
    ...
    let m = matrix_with_token(&server, "tok", vec!["!room:host"]);
    let mut c = ctx("", "files");
    c.attachments.push(crate::notify::Attachment { name: "a.png".to_string(), data: b"PNG".to_vec(), mime_type: "image/png".to_string() });
    c.attachments.push(crate::notify::Attachment { name: "b.txt".to_string(), data: b"hello".to_vec(), mime_type: "text/plain".to_string() });
  ...
  fn test_from_url_token_query_param() {
    ...
    let parsed = ParsedUrl::parse("matrix://user@localhost?mode=SLACK&format=markdown&token=mytoken").unwrap();
    let m = Matrix::from_url(&parsed).unwrap();
    ...
    let parsed2 = ParsedUrl::parse("matrix://_@localhost?mode=SLACK&token=mytoken").unwrap();
  ...
  fn test_from_url_t2bot_webhook() {
    ...
    let url = format!("https://webhooks.t2bot.io/api/v1/matrix/hook/{}/", token);

...
impl RocketChat {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let (host, webhook_token, user, password, raw_targets) = if url.user.is_none() && url.password.is_none() {
      ...
      match (&url.user, &url.password) {
        (Some(u), Some(p)) => {
          if mode_hint.as_deref() == Some("webhook") || p.contains('/') {
            ...
            let wh = if u.contains('/') { u.to_string() } else { format!("{}/{}", u, p) };
    ...
    let mut targets: Vec<String> = raw_targets.iter().filter(|s| !s.is_empty() && s.len() > 1).cloned().collect();
    ...
    if webhook_token.is_none() && targets.is_empty() {
      return None;
    }
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://rocket.chat"),
      setup_url: None,
      protocols: vec!["rocket", "rockets"],
      description: "Send via Rocket.Chat webhooks.",
...
impl Notify for RocketChat {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    for target in &self.targets {
      let channel = if target.starts_with('#') || target.starts_with('@') { target.clone() } else { format!("#{}", target) };
      ...
      if let Some(ref wh) = self.webhook_token {
        ...
        let payload = json!({ "text": text, "channel": channel });
        let resp = client.post(&url).header("User-Agent", APP_ID).basic_auth(u, Some(p)).json(&payload).send().await?;
        if !resp.status().is_success() {
          all_ok = false;
        }
      }
...
mod tests {
  ...
  fn rocketchat_webhook_for_mock(server: &MockServer, token: &str, targets: &[&str]) -> RocketChat {
    ...
    let url_str = format!("rocket://web/{}@{}:{}{}", token, addr.ip(), addr.port(), target_path);
  ...
  fn rocketchat_basic_for_mock(server: &MockServer, user: &str, pass: &str, targets: &[&str]) -> RocketChat {
    let addr = server.address();
    let target_path = targets.iter().map(|t| format!("/{}", t)).collect::<String>();
    let url_str = format!("rocket://{}:{}@{}:{}{}", user, pass, addr.ip(), addr.port(), target_path);
    let parsed = crate::utils::parse::ParsedUrl::parse(&url_str).unwrap();
    RocketChat::from_url(&parsed).unwrap()
  }
  ...
  async fn test_webhook_error_500_returns_false() {
    ...
    Mock::given(method("POST")).and(path("/hooks/web/errtoken")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_basic_auth_error_500_returns_false() {
    ...
    Mock::given(method("POST")).and(path("/api/v1/chat.postMessage")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_title_body_formatting() {
    ...
    let ctx = NotifyContext { title: "".into(), body: "Just the body".into(), ..Default::default() };

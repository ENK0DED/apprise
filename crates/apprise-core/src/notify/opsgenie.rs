...
impl OpsGenie {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    for (key, val) in &url.qsd {
      if let Some(ntype) = key.strip_prefix(':') {
        if !valid_types.contains(&ntype) {
          return None;
        }
    ...
    // Priority is accepted as-is; invalid values fall back to default at send time
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://www.opsgenie.com"),
      setup_url: None,
      protocols: vec!["opsgenie"],
      description: "Send alerts via OpsGenie.",
...
mod tests {
  ...
  fn test_from_url_eu_region() {
    let parsed = crate::utils::parse::ParsedUrl::parse("opsgenie://apikey/@user?region=eu").unwrap();
    let obj = OpsGenie::from_url(&parsed).unwrap();
  ...
  fn test_from_url_kwargs() {
    let parsed = crate::utils::parse::ParsedUrl::parse("opsgenie://?apikey=abc&to=user").unwrap();
    let obj = OpsGenie::from_url(&parsed).unwrap();
  ...
  fn test_from_url_multiple_targets() {
    let parsed = crate::utils::parse::ParsedUrl::parse("opsgenie://apikey/user@email.com/#team/*sche/^esc/%20/a").unwrap();
    let obj = OpsGenie::from_url(&parsed).unwrap();
  ...
  fn test_from_url_to_param_targets() {
    let parsed = crate::utils::parse::ParsedUrl::parse("opsgenie://apikey?to=#team,user").unwrap();
    let obj = OpsGenie::from_url(&parsed).unwrap();
  ...
  fn test_type_mappings() {
    ...
    let parsed = crate::utils::parse::ParsedUrl::parse("opsgenie://apikey/@user?:info=new").unwrap();
    assert!(OpsGenie::from_url(&parsed).is_some());

    let parsed = crate::utils::parse::ParsedUrl::parse("opsgenie://apikey/@user?:success=acknowledge").unwrap();
    assert!(OpsGenie::from_url(&parsed).is_some());
  }

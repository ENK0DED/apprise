...
impl Streamlabs {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    if let Some(name) = url.get("name") {
      if name.len() < 2 || name.len() > 25 || name.starts_with(' ') {
        return None;
      }
    }
    // Validate identifier if provided (regex: ^[^\s].{1,24}$)
    if let Some(ident) = url.get("identifier") {
      if ident.len() < 2 || ident.len() > 25 || ident.starts_with(' ') {
        return None;
      }
    }
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://streamlabs.com"),
      setup_url: None,
      protocols: vec!["strmlabs"],
      description: "Send alerts via Streamlabs.",
...
mod tests {
  ...
  fn test_from_url_fields() {
    let parsed = crate::utils::parse::ParsedUrl::parse("strmlabs://IcIcArukDQtuC1is1X1UdKZjTg118Lag2vScOmso").unwrap();
  ...
  fn test_currency_validation() {
    ...
    let parsed = crate::utils::parse::ParsedUrl::parse("strmlabs://IcIcArukDQtuC1is1X1UdKZjTg118Lag2vScOmso/?currency=USD").unwrap();
    assert!(Streamlabs::from_url(&parsed).is_some());

    // Invalid 4-letter currency
    let parsed = crate::utils::parse::ParsedUrl::parse("strmlabs://IcIcArukDQtuC1is1X1UdKZjTg118Lag2vScOmso/?currency=ABCD").unwrap();
    assert!(Streamlabs::from_url(&parsed).is_none());
  }
  ...
  fn test_call_validation() {
    for call in &["alerts", "donations", "ALERTS", "DONATIONS"] {
      let url = format!("strmlabs://IcIcArukDQtuC1is1X1UdKZjTg118Lag2vScOmso/?call={}", call);
    ...
    // Invalid call
    let parsed = crate::utils::parse::ParsedUrl::parse("strmlabs://IcIcArukDQtuC1is1X1UdKZjTg118Lag2vScOmso/?call=rms").unwrap();
    assert!(Streamlabs::from_url(&parsed).is_none());
  }
  ...
  fn test_alert_type_validation() {
    ...
    for at in &valid_types {
      let url = format!("strmlabs://IcIcArukDQtuC1is1X1UdKZjTg118Lag2vScOmso/?alert_type={}", at);
      let parsed = crate::utils::parse::ParsedUrl::parse(&url).unwrap();
      assert!(Streamlabs::from_url(&parsed).is_some(), "alert_type={} should be valid", at);
    }
    // Invalid alert_type
    let parsed = crate::utils::parse::ParsedUrl::parse("strmlabs://IcIcArukDQtuC1is1X1UdKZjTg118Lag2vScOmso/?alert_type=rms").unwrap();
    assert!(Streamlabs::from_url(&parsed).is_none());
  }
  ...
  fn test_name_validation() {
    ...
    let parsed = crate::utils::parse::ParsedUrl::parse("strmlabs://IcIcArukDQtuC1is1X1UdKZjTg118Lag2vScOmso/?name=t").unwrap();
    assert!(Streamlabs::from_url(&parsed).is_none());

    // Valid name (2 chars)
    let parsed = crate::utils::parse::ParsedUrl::parse("strmlabs://IcIcArukDQtuC1is1X1UdKZjTg118Lag2vScOmso/?name=tt").unwrap();
    assert!(Streamlabs::from_url(&parsed).is_some());
  }

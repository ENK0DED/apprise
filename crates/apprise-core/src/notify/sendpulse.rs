...
impl SendPulse {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let (client_id, client_secret, from_email, to) = if url.password.is_some() {
      ...
      let cid = url.path_parts.first()?.clone();
      ...
      let from = if user.contains('@') || user.contains('<') {
        user
      } else {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://sendpulse.com"),
      setup_url: None,
      protocols: vec!["sendpulse"],
      description: "Send email via SendPulse.",
...
impl Notify for SendPulse {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let mut payload =
      json!({ "html": ctx.body, "text": ctx.body, "subject": ctx.title, "from": { "name": "Apprise", "email": self.from_email }, "to": to_list });
...
mod tests {
  ...
  fn test_from_url_fields_user_at_domain() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sendpulse://user@example.com/client_id/cs1/").unwrap();
  ...
  fn test_from_url_fields_with_targets() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sendpulse://user@example.com/client_id/cs7/chris@example.com").unwrap();
    let sp = SendPulse::from_url(&parsed).unwrap();
    assert_eq!(sp.client_id, "client_id");
  ...
  fn test_from_url_query_format() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sendpulse://?id=ci&secret=cs&user=chris@example.com").unwrap();
    let sp = SendPulse::from_url(&parsed).unwrap();
  ...
  fn test_from_url_client_id_host_format() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sendpulse://client_id/cs1/?user=chris@example.ca").unwrap();
    let sp = SendPulse::from_url(&parsed).unwrap();

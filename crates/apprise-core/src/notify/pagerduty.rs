...
impl PagerDuty {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    let integration_key = url.host.clone().filter(|h| !h.is_empty()).or_else(|| url.get("integrationkey").map(|s| s.to_string()))?;
    let decoded_ik = urlencoding::decode(&integration_key).unwrap_or_default().into_owned();
    if decoded_ik.trim().is_empty() {
      return None;
    }
    ...
    if let Some(ref u) = url.user {
      ...
      if decoded_u.trim().is_empty() {
        return None;
      }
    }
    let apikey = url.user.clone().or_else(|| url.get("apikey").map(|s| s.to_string())).unwrap_or_default();
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://pagerduty.com"),
      setup_url: None,
      protocols: vec!["pagerduty"],
      description: "Send alerts via PagerDuty Events v2 API.",
...
impl Notify for PagerDuty {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    if let Some(ref c) = self.component {
      pd_payload["component"] = json!(c);
    }
    if let Some(ref g) = self.group {
      pd_payload["group"] = json!(g);
    }
    if let Some(ref c) = self.class {
      pd_payload["class"] = json!(c);
    }
...
mod tests {
  ...
  fn test_from_url_basic_fields() {
    let parsed = crate::utils::parse::ParsedUrl::parse("pagerduty://myroutekey@myapikey").unwrap();
  ...
  fn test_from_url_eu_region() {
    let parsed = crate::utils::parse::ParsedUrl::parse("pagerduty://myroutekey@myapikey?region=eu").unwrap();
    let obj = PagerDuty::from_url(&parsed).unwrap();
  ...
  fn test_from_url_all_params() {
    let parsed =
      crate::utils::parse::ParsedUrl::parse("pagerduty://?integrationkey=r&apikey=a&source=s&component=c&group=g&class=cl&click=http://localhost").unwrap();
    let obj = PagerDuty::from_url(&parsed).unwrap();
  ...
  fn test_from_url_with_group_class() {
    let parsed = crate::utils::parse::ParsedUrl::parse("pagerduty://routekey@apikey/ms/mc?group=mygroup&class=myclass").unwrap();
    let obj = PagerDuty::from_url(&parsed).unwrap();

...
impl MessageBird {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let api_key = url.password.clone().or_else(|| url.host.clone().filter(|h| !h.is_empty()))?;
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://www.messagebird.com"),
      setup_url: None,
      protocols: vec!["msgbird"],
      description: "Send SMS via MessageBird.",
...
mod tests {
  ...
  fn test_valid_urls() {
    let key = "a".repeat(25);
    let urls = vec![format!("msgbird://{}/15551232000", key), format!("msgbird://{}/?from=15551233000&to=15551232000", key)];
    for url in &urls {

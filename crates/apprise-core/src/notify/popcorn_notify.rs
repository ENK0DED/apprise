...
impl PopcornNotify {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    if !apikey.chars().any(|c| c.is_ascii_alphanumeric()) {
      return None;
    }
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "Popcorn Notify",
      service_url: Some("https://popcornnotify.com"),
      setup_url: None,
      protocols: vec!["popcorn"],
      description: "Send notifications via Popcorn Notify.",
...
impl Notify for PopcornNotify {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    for target in &self.targets {
      ...
      let resp = client
        .post("https://popcornnotify.com/notify")
        .header("User-Agent", APP_ID)
        .basic_auth(&self.apikey, Option::<&str>::None)
        .form(&params)
        .send()
        .await?;

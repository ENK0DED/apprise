...
mod tests {
    ...
    fn test_priority_mapping() {
        ...
        let parsed = crate::utils::parse::ParsedUrl::parse(
            &format!("gotify://host/{}?priority=low", token),
        ).unwrap();
        let g = Gotify::from_url(&parsed).unwrap();
        assert_eq!(g.priority, 1);

        let parsed = crate::utils::parse::ParsedUrl::parse(
            &format!("gotify://host/{}?priority=moderate", token),
        ).unwrap();
        let g = Gotify::from_url(&parsed).unwrap();
        assert_eq!(g.priority, 3);

        let parsed = crate::utils::parse::ParsedUrl::parse(
            &format!("gotify://host/{}?priority=normal", token),
        ).unwrap();
        let g = Gotify::from_url(&parsed).unwrap();
        assert_eq!(g.priority, 5);

        let parsed = crate::utils::parse::ParsedUrl::parse(
            &format!("gotify://host/{}?priority=high", token),
        ).unwrap();
        let g = Gotify::from_url(&parsed).unwrap();
        assert_eq!(g.priority, 8);

        let parsed = crate::utils::parse::ParsedUrl::parse(
            &format!("gotify://host/{}?priority=emergency", token),
        ).unwrap();
        let g = Gotify::from_url(&parsed).unwrap();
        ...
        // Numeric priority
        let parsed = crate::utils::parse::ParsedUrl::parse(
            &format!("gotify://host/{}?priority=7", token),
        ).unwrap();
        let g = Gotify::from_url(&parsed).unwrap();
        ...
        let parsed = crate::utils::parse::ParsedUrl::parse(
            &format!("gotify://host:8008/{}?priority=invalid", token),
        ).unwrap();
        let g = Gotify::from_url(&parsed).unwrap();
        assert_eq!(g.priority, 5);
    ...
    fn test_path_construction() {
        ...
        // With path prefix
        let parsed = crate::utils::parse::ParsedUrl::parse(
            &format!("gotify://host/a/path/ending/in/a/slash/{}", token),
        ).unwrap();
        let g = Gotify::from_url(&parsed).unwrap();
    ...
    async fn test_send_with_priority() {
        ...
        let addr = server.address();
        let url_str = format!(
            "gotify://{}:{}/{}?priority=high",
            addr.ip(), addr.port(), token
        );
        let parsed = crate::utils::parse::ParsedUrl::parse(&url_str).unwrap();

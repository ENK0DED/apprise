...
impl VoipMs {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let did = url.get("from").or_else(|| url.get("source")).map(|s| s.to_string()).or_else(|| url.path_parts.first().cloned())?;
    if did.is_empty() {
      return None;
    }
    // Validate DID has at least 11 digits
    let did_digits: String = did.chars().filter(|c| c.is_ascii_digit()).collect();
    if did_digits.len() < 11 {
      return None;
    }
    // Targets
    let mut targets: Vec<String> =
      if url.get("from").is_some() || url.get("source").is_some() { url.path_parts.clone() } else { url.path_parts.get(1..).unwrap_or(&[]).to_vec() };
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      service_name: "VoIP.ms",
      service_url: Some("https://voip.ms"),
      setup_url: None,
      protocols: vec!["voipms"],
      description: "Send SMS via VoIP.ms.",
...
mod tests {
  ...
  fn test_from_url_fields() {
    ...
    let parsed = ParsedUrl::parse("voipms://password:user@example.com/16138884444/16134442222").unwrap();
  ...
  fn test_from_url_query_params() {
    let parsed = ParsedUrl::parse("voipms://password:user@example.com/?from=16138884444&to=16134448888").unwrap();
    let v = VoipMs::from_url(&parsed).unwrap();
    assert_eq!(v.did, "16138884444");
  ...
  fn test_from_url_multiple_targets() {
    let parsed = ParsedUrl::parse("voipms://password:user@example.com/16138884444/16134442222/16134443333").unwrap();
    let v = VoipMs::from_url(&parsed).unwrap();

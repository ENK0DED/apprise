...
impl Fcm {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let mut targets: Vec<String> = url.path_parts.iter().filter(|s| !s.trim().is_empty()).cloned().collect();
...
impl Notify for Fcm {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    for target in &self.targets {
      ...
      let (to_field, to_value) = if let Some(stripped) = target.strip_prefix('#') { ("to", format!("/topics/{}", stripped)) } else { ("to", target.clone()) };

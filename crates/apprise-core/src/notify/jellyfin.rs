...
impl Jellyfin {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let apikey = url.user.clone().or_else(|| url.password.clone()).or_else(|| url.get("apikey").map(|s| s.to_string())).filter(|s| !s.is_empty())?;
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://jellyfin.org"),
      setup_url: None,
      protocols: vec!["jellyfin", "jellyfins"],
      description: "Send notifications via Jellyfin.",
...
impl Notify for Jellyfin {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    let resp = client
      ...
      .header("X-Emby-Authorization", format!("MediaBrowser Token=\"{}\"", self.apikey))
      .json(&payload)
      .send()
      .await?;
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["jellyfin://", "jellyfins://", "jellyfin://localhost", "jellyfin://:@/"];
    for url in &urls {
  ...
  async fn test_send_success() {
    ...
    Mock::given(method("POST")).and(path("/Notifications/Admin")).respond_with(ResponseTemplate::new(200)).expect(1).mount(&server).await;
  ...
  async fn test_send_server_error() {
    ...
    Mock::given(method("POST")).and(path("/Notifications/Admin")).respond_with(ResponseTemplate::new(500)).expect(1).mount(&server).await;
  ...
  async fn test_send_bizarre_status_code() {
    ...
    Mock::given(method("POST")).and(path("/Notifications/Admin")).respond_with(ResponseTemplate::new(418)).expect(1).mount(&server).await;
  ...
  async fn test_send_auth_header() {
    ...
    Mock::given(method("POST"))
      .and(path("/Notifications/Admin"))
      .and(wiremock::matchers::header("X-Emby-Authorization", "MediaBrowser Token=\"my-api-key\""))
      .respond_with(ResponseTemplate::new(200))
      .expect(1)
      .mount(&server)
      .await;
  ...
  #[test]
  fn test_valid_urls() {
    let urls = vec!["jellyfin://l2g@localhost", "jellyfins://l2g:password@localhost"];
    for url in &urls {

...
enum SlackMode {
  ...
  Webhook { token_a: String, token_b: String, token_c: String, channels: Vec<String> },
  /// Bot token — access_token + channels
  Bot { access_token: String, channels: Vec<String> },
}
...
impl Slack {
  pub fn from_url(url: &ParsedUrl) -> Option<Self> {
    ...
    let bot_name = url.user.clone().or_else(|| url.get("user").map(|s| s.to_string()));
    ...
    let is_bot_token = token_parts
      .first()
      .map(|t| t.starts_with("xoxb-") || t.starts_with("xoxe.xoxb-") || t.starts_with("xoxe.xoxp-") || t.starts_with("xoxp-"))
      .unwrap_or(false);

    let forced_webhook = mode_hint.as_deref() == Some("hook") || mode_hint.as_deref() == Some("webhook") || mode_hint.as_deref() == Some("w");
...
impl Notify for Slack {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    ...
    match &self.mode {
      SlackMode::Webhook { token_a, token_b, token_c, channels } => {
        let webhook_url = {
          let default = format!("https://hooks.slack.com/services/{}/{}/{}", token_a, token_b, token_c);
          #[cfg(test)]
          {
        ...
        let targets: Vec<Option<&String>> = if channels.is_empty() { vec![None] } else { channels.iter().map(Some).collect() };
        let mut all_ok = true;
        for target in targets {
          ...
          let resp = client.post(&webhook_url).header("User-Agent", APP_ID).json(&payload).send().await?;
      ...
      SlackMode::Bot { access_token, channels } => {
        ...
        for channel in channels {
          ...
          let resp =
            client.post(&api_post_url).header("User-Agent", APP_ID).header("Authorization", format!("Bearer {}", access_token)).json(&payload).send().await?;
          if !resp.status().is_success() {
            all_ok = false;
          }
          ...
          for att in &ctx.attachments {
            ...
            let form = reqwest::multipart::Form::new().text("channels", channel.clone()).part("file", part);
            let upload_url = {
            ...
            let _ = client.post(&upload_url).header("Authorization", format!("Bearer {}", access_token)).multipart(form).send().await;
          }
        }
        Ok(all_ok)
      }
...
mod tests {
  ...
  fn bot_slack(server: &MockServer, channels: Vec<&str>) -> Slack {
    ...
    let ch: Vec<String> = channels.iter().map(|s| s.to_string()).collect();
    Slack {
      mode: SlackMode::Bot { access_token: "xoxb-1234-1234-abc124".to_string(), channels: ch },
      verify_certificate: false,
      tags: vec![],
      bot_name: Some("TestBot".to_string()),
      webhook_url_override: None,
  ...
  async fn test_bot_post_with_bearer_token() {
    ...
    Mock::given(method("POST"))
      ...
      .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"ok": true, "message": ""})))
      .expect(1)
      .mount(&server)
      .await;
  ...
  async fn test_bot_attachment_upload() {
    ...
    Mock::given(method("POST"))
      .and(path("/api/chat.postMessage"))
      .and(header("Authorization", "Bearer xoxb-1234-1234-abc124"))
      .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({"ok": true, "channel": "C123456"})))
      .expect(1)
      .mount(&server)
      .await;
    ...
    c.attachments.push(crate::notify::Attachment { name: "apprise-test.gif".to_string(), data: b"GIF89a".to_vec(), mime_type: "image/gif".to_string() });
  ...
  async fn test_webhook_attachment_upload() {
    ...
    c.attachments.push(crate::notify::Attachment { name: "test.png".to_string(), data: b"PNG".to_vec(), mime_type: "image/png".to_string() });
  ...
  async fn test_bot_connection_refused_returns_error() {
    let slack = Slack {
      mode: SlackMode::Bot { access_token: "xoxb-1234-1234-abc124".to_string(), channels: vec!["#chan".to_string()] },
      verify_certificate: false,
      tags: vec![],
      bot_name: None,
      webhook_url_override: None,
  ...
  fn test_webhook_mode_from_url() {
    let parsed = ParsedUrl::parse("slack://T1JJ3T3L2/A1BRTD4JD/TIiajkdnlazkcOXrIdevi7FQ/#channel").unwrap();
  ...
  fn test_bot_mode_from_url() {
    let parsed = ParsedUrl::parse("slack://username@xoxb-1234-1234-abc124/#nuxref").unwrap();
    let slack = Slack::from_url(&parsed).unwrap();
  ...
  fn test_bot_mode_rotating_token() {
    ...
    let parsed = ParsedUrl::parse("slack://username@xoxe.xoxb-1234-1234-abc124/#nuxref").unwrap();
    let slack = Slack::from_url(&parsed).unwrap();
    assert!(matches!(slack.mode, SlackMode::Bot { .. }));
  }
  ...
  fn test_xoxe_xoxp_rotating_token() {
    ...
    let parsed = ParsedUrl::parse("slack://username@xoxe.xoxp-1234-1234-abc124/#nuxref").unwrap();
    let slack = Slack::from_url(&parsed).unwrap();
    assert!(matches!(slack.mode, SlackMode::Bot { .. }));
  }
  ...
  fn test_forced_hook_mode_with_webhook_tokens() {
    let parsed = ParsedUrl::parse("slack://?token=T1JJ3T3L2/A1BRTD4JD/TIiajkdnlazkcOXrIdevi7FQ/&to=#chan&mode=hook").unwrap();
    let slack = Slack::from_url(&parsed).unwrap();
    assert!(matches!(slack.mode, SlackMode::Webhook { .. }));
  }
  ...
  fn test_forced_bot_mode_with_webhook_tokens_rejected() {
    ...
    let parsed = ParsedUrl::parse("slack://?token=T1JJ3T3L2/A1BRTD4JD/TIiajkdnlazkcOXrIdevi7FQ/&to=#chan&mode=bot").unwrap();
    assert!(Slack::from_url(&parsed).is_none());
  }

  #[test]
  fn test_invalid_mode_rejected() {
    let parsed = ParsedUrl::parse("slack://T1JJ3T3L2/A1BRTD4JD/TIiajkdnlazkcOXrIdevi7FQ/?mode=invalid").unwrap();
    assert!(Slack::from_url(&parsed).is_none());
  }

  #[test]
  fn test_bot_name_from_url_user() {
    let parsed = ParsedUrl::parse("slack://mybot@xoxb-1234-1234-abc124/#nuxref").unwrap();
    let slack = Slack::from_url(&parsed).unwrap();
  ...
  fn test_bot_name_from_user_query_param() {
    let parsed = ParsedUrl::parse("slack://?token=xoxb-1234-1234-abc124&to=#nuxref&user=testbot").unwrap();
    let slack = Slack::from_url(&parsed).unwrap();
  ...
  fn test_token_from_query_param() {
    let parsed = ParsedUrl::parse("slack://bot@_/#nuxref?token=T1JJ3T3L2/A1BRTD4JD/TIiajkdnadfdajkjkfl/").unwrap();
    let slack = Slack::from_url(&parsed).unwrap();
    assert!(matches!(slack.mode, SlackMode::Webhook { .. }));
  }
  ...
  fn test_invalid_token_a_rejected() {
    let parsed = ParsedUrl::parse("slack://username@-INVALID-/A1BRTD4JD/TIiajkdnlazkcOXrIdevi7FQ/#cool").unwrap();
    assert!(Slack::from_url(&parsed).is_none());
  }

  #[test]
  fn test_invalid_token_b_rejected() {
    let parsed = ParsedUrl::parse("slack://username@T1JJ3T3L2/-INVALID-/TIiajkdnlazkcOXrIdevi7FQ/#great").unwrap();
    assert!(Slack::from_url(&parsed).is_none());
  }

  #[test]
  fn test_invalid_token_c_rejected() {
    let parsed = ParsedUrl::parse("slack://username@T1JJ3T3L2/A1BRTD4JD/-INVALID-/#channel").unwrap();
    assert!(Slack::from_url(&parsed).is_none());
  }

...
impl SendGrid {
  ...
  pub fn static_details() -> ServiceDetails {
    ServiceDetails {
      ...
      service_url: Some("https://sendgrid.com"),
      setup_url: None,
      protocols: vec!["sendgrid"],
...
impl Notify for SendGrid {
  ...
  async fn send(&self, ctx: &NotifyContext) -> Result<bool, NotifyError> {
    let personalizations: Vec<_> = self
      .to
      .iter()
      .map(|t| {
...
mod tests {
  ...
  fn test_invalid_urls() {
    let urls = vec!["sendgrid://", "sendgrid://:@/", "sendgrid://abcd", "sendgrid://abcd@host", "sendgrid://invalid-api-key+*-d:user@example.com"];
    for url in &urls {
  ...
  fn test_from_url_fields_basic() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sendgrid://abcd:user@example.com").unwrap();
  ...
  fn test_from_url_fields_with_target() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sendgrid://abcd:user@example.com/newuser@example.com").unwrap();
    let sg = SendGrid::from_url(&parsed).unwrap();
    assert_eq!(sg.apikey, "abcd");
    assert_eq!(sg.from_email, "user@example.com");
  ...
  fn test_from_url_cc_bcc() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sendgrid://abcd:user@example.com/newuser@example.com?bcc=l2g@nuxref.com&cc=abc@test.org").unwrap();
    let sg = SendGrid::from_url(&parsed).unwrap();
  ...
  fn test_from_url_to_param() {
    let parsed = crate::utils::parse::ParsedUrl::parse("sendgrid://abcd:user@example.com/newuser@example.com?to=l2g@nuxref.com").unwrap();
    let sg = SendGrid::from_url(&parsed).unwrap();

...
use apprise_core::{
    Apprise, NotifyContext, SendResult,
    NotifyType, NotifyFormat, StorageMode,
    registry, default_storage_path,
    storage::PersistentStore,
    utils::parse::mask_url,
};
...
async fn main() {
    ...
    if let Some(Commands::Storage(storage_args)) = &cli.command {
        let storage_path = storage_args.storage_path.clone()
        ...
        let store = PersistentStore::new(
            ...
            storage_args.storage_uid_length as usize,
            storage_args.storage_prune_days,
            storage_mode,
        );
        match storage_args.action.as_str() {
            "list" => {
                ...
                else { for e in entries { println!("{}\t{}\t{}", e.uid, e.url_hash, e.last_used.format("%Y-%m-%d %H:%M:%S")); } }
            }
            "prune" => println!("Pruned {} entries.", store.prune().await),
            "clean" => println!("Cleaned {} entries.", store.clean().await),
            other => { eprintln!("Unknown storage action: {}", other); std::process::exit(2); }
        }
        return;
    }
    ...
    if cli.details {
        ...
        println!("{}", "-".repeat(100));
        for d in all { println!("{:<30} {:<50} {}", d.service_name, d.description, d.protocols.join(", ")); }
        return;
    }
    ...
    if cli.dry_run {
        ...
        for d in apprise.details() { println!("  {} ({})", d.service_name, d.protocols.join(", ")); }
        return;
    }
    ...
    if body.is_empty() { eprintln!("No message body provided."); std::process::exit(2); }
    ...
    // Send
    let result = if cli.disable_async {
        apprise.notify_sequential(&ctx).await
    } else {
        apprise.notify(&ctx).await
    };
